...
## 2026-07-19 — Właściciel formalnie zamknął falę Production Schema Migration Orchestrator

- **Stan wejściowy:** implementer dostarczył `PRODUCTION MIGRATION ORCHESTRATOR — CANDIDATE COMPLETE` (ADR-108); niezależny review wydał `APPROVE WITH MINOR/P2` (0 CRITICAL / 0 MAJOR).
- **Merge i checkpoint:** właściciel przyjął merge PR #11 jako `7faf62e5f71c838c20e00e61121ea052f4bf9348` (rodzice `889567507a82097f1e4f5d62a2d2bc1f74d8e936` i zatwierdzony head `9a10bf5fc364534ee08d4329753cb1a537e10a76`) oraz zielony post-merge checkpoint: pełna suita `1630/1630` zero skipped, exact-once `1630`, orchestrator `58/58`, targeted `224/224`, regresja `218/218`, QA `30/30`, harnessy `13/13+13/13`, acceptance `63/63`, compile/diff PASS. Właściciel przyjął też uczciwy zapis interferencji pierwszego przebiegu (`1628/1630`, izolacja `2/2`, czysty rerun `1630/1630`) jako operational test interference / procedural lesson, nie defekt produktu.
- **Formalna decyzja człowieka:** właściciel ustawił `Production Schema Migration Orchestrator = CLOSED — APPROVED WITH MINOR/P2` (ADR-109). Implementer nie zamknął fali samodzielnie.
...
## 2026-07-24 — Właściciel autoryzował merge PR #24 i wąską synchronizację dokumentacji C3

- **Decyzja człowieka:** po pełnym niezależnym review `APPROVE WITH MINOR/P2`, poprawce MINOR (`47/47`→`48/48`) i wąskim re-review `APPROVE` właściciel autoryzował merge kodowego PR #24 metodą merge commit, synchronizację `main` ff-only i post-merge weryfikację; następnie osobno autoryzował wyłącznie dokumentacyjną synchronizację (nowy branch, jeden commit, push, niedraftowy PR do `main`) — bez merge tego PR.
...
## 2026-07-24 — Właściciel autoryzował merge PR #26 i wąską synchronizację dokumentacji C4

- **Decyzja człowieka:** po pełnym niezależnym review `APPROVE WITH P2` (0 MAJOR / 0 MINOR) i osobnym review integralności PR `APPROVE WITH P2` właściciel autoryzował merge kodowego PR #26 metodą merge commit, synchronizację `main` ff-only i post-merge weryfikację; następnie osobno autoryzował wyłącznie dokumentacyjną synchronizację (nowy branch, jeden commit, push, niedraftowy PR do `main`) — bez merge tego PR.
- **Wynik merge:** commit implementacji `6a97620048d1099b9c1f0da29ec343ae12a54559` (`feat: add autonomous content decision layer`), PR #26, merge commit `7eb93ba93b131d0a9a3c33e7d8495500afaa721f` (rodzice `b6abd60583b371b4501551735f4d67dffd7f2944` i `6a97620048d1099b9c1f0da29ec343ae12a54559`). Post-merge: C4 `23/23`, C2+C3 `48/48`, pełna suita `1994/1994`, collect/unique `1994`, duplikaty/skipped/xfail/errors `0`, delta `+23`, compile/diff PASS; koszt `0.000000 USD`.
- **Twarde granice:** produkcja wyłącznie read-only/immutable, nadal `0020` (kod `0024`), bez migracji, `autonomous_decisions` nieobecna w produkcji; zero realnego API/SDK/sieci/browsera/publikacji/controlled-live; bez zmian kodu/testów/migracji; bez C5/PRE-C5; dokumentacyjny PR NIE jest mergowany i czeka na wąski niezależny review.
- **Granica statusu:** dozwolony status to `C4 CODE MERGED — POST-MERGE VERIFIED — AWAITING DOCUMENTATION SYNC MERGE`; `C4 FULLY CLOSED` dopiero po review i merge dokumentacyjnego PR. Osiem P2 pozostaje otwartych; P2-1/P2-2/P2-3 są gate'ami przed C5. Sześć nieblokujących ustaleń review pozostaje zapisami review, nie blockerami.

## 2026-08-08 — Właściciel zlecił skonsolidowaną falę bramek technicznych PRE-C5

- **Decyzja człowieka:** wykonać jedną skonsolidowaną falę zamykającą wyłącznie sześć wcześniej potwierdzonych wymagań przed C5: niezależną kontrolę jakości draftu zamiast self-reportu, deterministyczny admission research corpus, pełny real-compatible E3 lineage wymagany przez CONTENT, wcześniejszy gate paid usage/terminalizacji, wcześniejszy gate CONTENT heartbeat/lease oraz włączenie 3–5 zatwierdzonych krótkich przykładów stylu ARTICLE z trwałymi example IDs i audytowalnym fingerprintem.
- **Jawnie wyłączone z zakresu przez właściciela:** Notes profile `PROVISIONAL`, nazewnictwo Pangram/humanizer/detector, stary `CLAUDE_INSTRUKCJA_NATURALNEGO_PISANIA.md` (pozostaje tracked i nieusuwany), kosmetyka promptu, historyczna dokumentacja, SQLite URI helper poza koniecznością, heartbeat flake poza dotkniętym flow oraz provider/model/pricing availability.
- **Twarde granice nadane przez człowieka:** zero realnego providera i zero sieci — wyłącznie fake SDK, fake callery i deterministyczne fixtures; syntetyczne usage/koszt zamiast prawdziwej operacji płatnej; nowe testy wyłącznie na nowych tymczasowych bazach SQLite; produkcyjna `data/agent.db` tylko `mode=ro&immutable=1` z `PRAGMA query_only=ON` i bez testu ochrony przez rzeczywisty zapis; prywatny korpus stylu bez modyfikacji, bez commitowania i bez przenoszenia; bez stage/commita/pushu/PR/merge.
- **Wymagany status:** implementer może wydać wyłącznie `PRE-C5 TECHNICAL GATES CANDIDATE COMPLETE — AWAITING INDEPENDENT REVIEW` albo `PRE-C5 TECHNICAL GATES BLOCKED`. Implementer NIE zamyka PRE-C5 QUALITY GATE, NIE ogłasza gotowości C5 i NIE ogłasza live readiness.
- **Zachowane decyzje:** C4 pozostaje `FULLY CLOSED` i nie jest otwierane; C5 i PRE-C5 QUALITY GATE pozostają `NOT STARTED`; produkcja pozostaje na `0020` bez autoryzacji migracji; osiem wcześniejszych P2 pozostaje otwartych, a P2-1/P2-2/P2-3 nadal są gate'ami przed C5.

## 2026-08-09 — Właściciel zlecił jedyną falę naprawczą PRE-C5 po `REJECT — MAJOR`

- **Decyzja człowieka:** po niezależnym review z werdyktem `REJECT — MAJOR` wykonać dokładnie jedną falę naprawczą obejmującą wyłącznie cztery autorytatywne findings (PRE5-RV-01 fakty spoza evidence, PRE5-RV-02 admission poza runtime, PRE5-RV-03 nieporównywalny klucz syndykacji, PRE5-RV-04 lease krótszy niż writer call) oraz otwarty gate paid CONTENT z pierwotnego zakresu PRE-C5. Po naprawie ma nastąpić dokładnie jeden niezależny re-review.
- **Jawnie wyłączone przez właściciela:** PRE5-RV-05 (trigger SQL floor), PRE5-RV-06 (niepełny PSL), PRE5-RV-07 (pre-existing duplicate node ID), Notes profile, nazewnictwo Pangram/humanizer, stary `CLAUDE_INSTRUKCJA_NATURALNEGO_PISANIA.md`, historyczne etykiety dokumentacyjne, kosmetyka promptu i pozostałe P2.
- **Jawnie odłożone do osobnej fali po APPROVE:** `claude-opus-5`, `claude-sonnet-5`, `claude-fable-5`, realny composition root Anthropic, aktualne ceny i model IDs oraz dynamiczny model routing.
- **Twarde granice nadane przez człowieka:** zero aplikacyjnej sieci, zero realnego API, zero live SDK Anthropic, zero browsera, zero publikacji, zero kosztu; wyłącznie fake SDK i callery; zapisy tylko do nowych temp DB; produkcyjna baza wyłącznie immutable/read-only; bez zmian sekretów; bez stage/commita/pushu/PR/merge.
- **Wymagany status:** implementer może wydać wyłącznie `PRE-C5 TECHNICAL GATES REPAIR CANDIDATE COMPLETE — AWAITING RE-REVIEW` albo `PRE-C5 TECHNICAL GATES REPAIR BLOCKED`, i nie wykonuje re-review w tej samej sesji. Gate paid CONTENT nie może być oznaczony `CLOSED` bez bezpośredniego content-specific dowodu na fake callerze.
- **Zachowane decyzje:** C4 pozostaje `FULLY CLOSED`; C5 i PRE-C5 QUALITY GATE pozostają `NOT STARTED`; produkcja pozostaje na `0020` bez autoryzacji migracji; wcześniejsze P2 pozostają otwarte.

## 2026-08-09 — Właściciel zlecił wąską falę PRE5-RR-01

- **Decyzja człowieka:** po drugim `REJECT — MAJOR` zamknąć dokładnie PRE5-RR-01 przez kompletne, fail-closed ARTICLE claim accounting oraz atomowe zaksięgowanie i terminalizację paid CONTENT over-cap bez oczekiwania na reaper.
