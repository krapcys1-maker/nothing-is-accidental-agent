...
> **That blocker is closed.** The 0.485760 research reservation was reconciled
> as `CHARGED_KNOWN` at `0.247745` from the provider console, and ADR-154
> (schema 0042) now lets owner conservative adjudication reach research
> attempts, so an unknown research charge no longer shuts the gate for the
> whole account.
