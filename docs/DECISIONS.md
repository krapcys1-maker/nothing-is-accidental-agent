...
### ADR-107: Formalne zamknięcie WAVE E2-C po niezależnym review, merge PR #9 i post-merge checkpointcie

- **Data/status:** 2026-07-19; ACCEPTED — formalna decyzja właściciela. **E2-C = `CLOSED — APPROVED WITH MINOR/P2`**. E1, E2-A, E2-B i E2-C są zamknięte; cały Etap 2 pozostaje **`IN PROGRESS`**; controlled-live = **`NOT READY`**; następna operacja techniczna = **`NOT STARTED`**.
- **Implementer:** ADR-106 dostarczył `E2-C CANDIDATE COMPLETE — AWAITING INDEPENDENT REVIEW`: capability ze storage po atomowym zużyciu dokładnego approval L1, sealed factory w granicach wspieranego runtime, strict boolean YAML domyślnie `false` i immutable `BoundHttpTarget` używany przez transport bez ponownego DNS. Implementer nie zatwierdził i nie zamknął fali samodzielnie.
- **Niezależny review:** werdykt `APPROVE WITH MINOR/P2`. Review potwierdził zamknięcie technicznego zakresu i pozostawił jawny, nieblokujący backlog P2.
- **Merge:** PR #9 został zmergowany metodą merge commit jako `ff323746c35f733507a7b0a30837ebf645020b2b`; rodzice: `cf3d083bc66387b2fa35f1dce4435ad0eb527b21` i zatwierdzony head PR `c508646be011a51fe973e42c197d23ddbca7fcd6`.
- **Post-merge checkpoint:** collect `1572`, exact-once `1572`, pełna suita `1572/1572`, zero skipped, partycje `378+389+394+411`, targeted `478/478`, podstawowy zestaw E2-C/E2-B/adapter/config `101/101`, harness E2-C `13/13`, harness E2-B `13/13`, `compileall` PASS i `git diff --check` PASS. Produkcyjna baza pozostała niezmieniona; koszt `0.000000 USD`; zero realnego DNS, socketów, HTTP, API, providera, browsera i publikacji.
- **Decyzja właściciela:** na podstawie implementacji, niezależnego review, merge PR #9 i zielonego checkpointu właściciel formalnie ustawia **E2-C = `CLOSED — APPROVED WITH MINOR/P2`**.
- **Status findings:** `E2B-F-01` = **`TECHNICALLY CLOSED IN MERGED E2-C`**; `E2B-F-02` = **`TECHNICALLY CLOSED IN MERGED E2-C`**; `E2B-F-03` = **`TECHNICALLY CLOSED IN MERGED E2-C`**. Realny transport jest chroniony w granicach wspieranego runtime i composition roots; host binding wiąże transport z adresem, który przeszedł politykę; globalna konfiguracja nie zastępuje trwałego approval L1 konkretnego joba. Nie jest to absolutna ochrona przed autorem dowolnego kodu Python.
- **Jawny backlog bez naprawy:** `E2B-F-04` pozostaje `MINOR/P2`; `E2B-F-05` pozostaje `P2`; `E2B-OBS-02` pozostaje informational; `PR8-DOC-P2-01` pozostaje `P2`; `F-DOC-01` pozostaje `P2` dokładności raportowania; `F-01-RESIDUAL` pozostaje `P2` poza wspieranym runtime. Ten ADR nie zmienia statusu ani implementacji żadnego P2.
- **Granice zamknięcia:** runtime kodu wymaga schema `0018`; produkcja pozostaje byte-identical na schema `0014`, 14 migracjach, SHA-256 `9906AFBFB580BE8F576A6449B0930C41ED964FED814D99C947D1C28C5B060836`, `364544 B`, integrity `ok`, FK `0`, bez sidecarów. Produkcji nie migrowano; prawdziwego Fetch ani realnego staged A1/A2/B nie wykonano; browser i publikacja pozostają nieautoryzowane.
- **Następna operacja:** `NOT STARTED`. Może zostać osobno autoryzowana i dopiero wtedy objąć snapshot produkcyjnej bazy, kontrolowaną migrację `0014→0018`, osobną zgodę właściciela, jeden minimalny realny controlled Fetch, brak automatycznego retry i pełny audit trwałego wyniku. Ten ADR nie autoryzuje migracji ani realnego Fetch.
- **Granica dokumentu:** ADR formalizuje zamknięcie technicznej fali E2-C. Sam dokumentacyjny PR z ADR-107 pozostaje kandydatem do niezależnego review; nie zamyka całego Etapu 2, nie rozpoczyna następnej operacji i nie autoryzuje działania zewnętrznego.

### ADR-108: Jeden kontrolowany orchestrator produkcyjnej drabiny schema 0014→0018

- **Data/status:** 2026-07-19; decyzja implementacyjna właściciela dla wąskiej fali naprawczej. `PRODUCTION MIGRATION ORCHESTRATOR — CANDIDATE COMPLETE, AWAITING INDEPENDENT REVIEW`. Nie jest to `APPROVE`, zamknięcie Etapu 2, autoryzacja migracji produkcji ani gotowość controlled-live.
- **Root causes:** cztery osobne CLI jednostopniowe nie tworzyły jednej właścicielskiej operacji `0014→0018`; żaden nie wiązał wykonania z dokładnym path/SHA/size i nowym snapshotem; nie istniała niezależna rewalidacja trwałego pliku po snapshotcie i bezpośrednio przed writable open; historyczny mechanizm Stage 1 dopuszczał część sidecarów i nie był kontraktem tej fali; QA runtime zatrzymał się na założeniu `0017`.
- **Kontrakt CLI:** jedyny wspierany entrypoint to `scripts/migrate_production_schema_0014_to_0018.py`. Obowiązkowe: `--db-path`, `--expected-sha256`, `--expected-size`, `--expected-from-version 0014_provider_attempt_reconciliation`, `--target-version 0018_controlled_fetch_lifecycle`, `--snapshot-path` oraz `--confirm-production-schema-migration-0014-to-0018`. Brak lub inna wartość = STOP bez mutacji. Nie powstał migrator arbitralnych wersji.
- **Preflight/tożsamość:** absolutny kanoniczny regular file, brak symlink/junction/hardlink aliasu, stabilne `st_dev/st_ino` + mtime/ctime + SHA/size, exact canonical ledger, fizyczne obiekty schema zgodne z bieżącym szczeblem, integrity `ok`, FK `0`, brak sidecarów i pełna istniejąca sonda quiescence proces/task/handle. Start i resume są dozwolone wyłącznie dla spójnych `0014–0017`; przyszłość, brak/duplikaty/niekanoniczny ledger i schema mismatch kończą się fail-closed.
- **Snapshot i TOCTOU:** target snapshotu musi być nowy, w odrębnym katalogu poza repo, otwierany `xb`; copy jest fsyncowane, hashowane, sprawdzane immutable, byte-identical oraz ledger/schema/integrity/FK. Po snapshotcie źródło jest odczytywane od nowa. Dwa jawne okna interposition poprzedza ostatni pełny file-state gate wykonany bezpośrednio przed `mode=rw`; writable handle ponownie sprawdza ledger i file identity przed jakimkolwiek PRAGMA zapisowym lub migracją. Drift daje `STALE_DATABASE_STATE`.
- **Sidecary:** każdy istniejący `<db>-wal`, `<db>-shm` lub `<db>-journal` zatrzymuje operację przed snapshotem/writable open z kodem `WAL_PRESENT`, `SHM_PRESENT` lub `JOURNAL_PRESENT`; wielokrotne sidecary są raportowane łącznie. Nic nie jest usuwane, scalane ani traktowane jako checkpoint.
- **Transakcje/recovery:** istniejący `apply_migrations` zachowuje transakcję `BEGIN IMMEDIATE → SQL → ledger → COMMIT` per krok. Kontrolowany hak testowy działa wewnątrz tej transakcji przed ledgerem; jego wyjątek wycofuje bieżący schema+ledger. Cała sekwencja nie udaje jednej transakcji: ukończone wcześniejsze kroki są trwałe, raport podaje `durable_version` i `next_migration`. Resume `0015–0017` wymaga nowego CLI approval, bieżącego SHA/size i nowego snapshotu; zero auto-retry. `0018` zwraca `ALREADY_AT_TARGET` bez snapshotu i bez mutacji.
- **Finalna walidacja:** po close wymaga braku sidecarów, immutable reopen, 18 unikalnych wersji z last `0018`, kompletu tabel/indexów/triggerów `0015–0018`, integrity `ok`, FK `0` i akceptacji exact runtime gate bez zmiany SHA. Failure daje `MIGRATION_RESULT_NEEDS_OPERATOR_ASSESSMENT`; snapshot nigdy nie jest automatycznie przywracany.
- **QA i dowód:** `runtime_schema_gate_disproof.py` potwierdza `0017 reject`, `0018 accept`, przyszłą wersję reject, brak automigracji i brak mutacji na refusal (`30/30`). Orchestrator `58/58`; pełna suita `1630/1630`, exact-once `1630`, partycje `390+398+412+430`; harnessy E2-B/E2-C `13/13+13/13`; `compileall app scripts` i `git diff --check` PASS.
- **Bezpieczeństwo/granice:** wszystkie zapisy testowe wyłącznie do nowych temp DB i snapshotów. Produkcja wyłącznie file hash + `mode=ro&immutable=1`, bez migracji: `0014`, 14 migracji, SHA `9906AFBFB580BE8F576A6449B0930C41ED964FED814D99C947D1C28C5B060836`, `364544 B`, integrity `ok`, FK `0`, sidecary `0`. Zero realnego DNS/socketu/HTTP/API/providera/browsera/publikacji i koszt `0.000000 USD`. Migracje SQL `0015–0018`, controlled Fetch i otwarte P2 pozostały nietknięte.

### ADR-109: Formalne zamknięcie Production Schema Migration Orchestrator po niezależnym review, merge PR #11 i post-merge checkpointcie

- **Data/status:** 2026-07-19. `PRODUCTION SCHEMA MIGRATION ORCHESTRATOR = CLOSED — APPROVED WITH MINOR/P2`. Zamknięcie dotyczy wyłącznie tej wąskiej fali; nie autoryzuje rzeczywistej migracji produkcji, nie wykonuje jej, nie zamyka Etapu 2 i nie czyni controlled-live gotowym.
- **Implementer:** dostarczył kandydata i ogłosił wyłącznie `PRODUCTION MIGRATION ORCHESTRATOR — CANDIDATE COMPLETE` (ADR-108). Implementer nie zatwierdził i nie zamknął fali samodzielnie.
- **Niezależny review:** `APPROVE WITH MINOR/P2` — 0 CRITICAL, 0 MAJOR; potwierdzone usunięcie czterech blockerów (jeden wspierany orchestrator `0014→0018`; kontrola aktualności/tożsamości pliku łącznie z gate bezpośrednio przed writable open i re-weryfikacją na otwartym uchwycie; bezwzględny STOP na WAL/SHM/journal bez usuwania; QA runtime zaktualizowane `0017→0018`); 59 niezależnych kontrprób na temp DB, w tym podmiany pliku w trzech oknach, sidecary w trzech oknach, awarie per krok, resume, anomalie ledger, junction/hardlink i realny CLI z domyślną sondą.
- **Merge:** PR #11 zmergowany metodą merge commit jako `7faf62e5f71c838c20e00e61121ea052f4bf9348`; rodzice: `889567507a82097f1e4f5d62a2d2bc1f74d8e936` (base `main`) i `9a10bf5fc364534ee08d4329753cb1a537e10a76` (dokładnie zatwierdzony head). Rzeczywisty diff: 28 plików, `+2182/−37`.
- **Post-merge checkpoint:** collect `1630`; exact-once `1630` unikalnych node ID (case-sensitive), 0 duplikatów; pełna suita `1630/1630`, zero skipped; orchestrator `58/58`; targeted `224/224`; dodatkowa regresja `218/218`; runtime schema QA `30/30`; harness E2-B `13/13`; harness E2-C `13/13`; acceptance orchestratora `63/63`; `compileall app scripts` i `git diff --check` PASS; produkcja niezmieniona; koszt `0.000000 USD`; zero realnego DNS/socketów/HTTP/API/providera/browsera/publikacji. Pierwszy przebieg pełnej suity dał `1628/1630`: dwa nieproduktowe failures dotyczyły wyłącznie reason code sondy quiescence (`PROCESSES_PRESENT` zamiast `DB_HANDLES_PRESENT`) przy równolegle działających procesach acceptance; flow zatrzymał się fail-closed; testy w izolacji `2/2`; czysty rerun `1630/1630`; defektu zmergowanego kodu nie zidentyfikowano. Klasyfikacja: operational test interference / procedural lesson — acceptance z sondą procesów nie uruchamiać równolegle z testami LA-02.
- **Decyzja właściciela:** na podstawie implementacji, niezależnego review, merge PR #11 i zielonego post-merge checkpointu właściciel formalnie ustawił `Production Schema Migration Orchestrator = CLOSED — APPROVED WITH MINOR/P2`.
- **Findings:** zaakceptowane P2, nienaprawiane w tej fali: `F-PR11-01` (błędna statystyka diffu w raporcie implementera `+2251/−106`; prawidłowo 28 plików `+2182/−37`), `F-PR11-02` (nieaktualna liczba testów w README — istniejący dokumentacyjny P2 poza minimalnym zakresem), `F-PR11-03` (`--expected-from-version` przy resume z `0015–0017` nadal przyjmuje literalny początek drabiny `0014`; barierą pozostają aktualny SHA, aktualny rozmiar, ledger, nowy snapshot, jawne potwierdzenie i resume requirements w raporcie). Obserwacje zachowane: `OBS-PR11-01` (failpoint przed finalną walidacją daje STOP z jawnym durable `0018`, nie fałszywy sukces), `OBS-PR11-02` (raport po awarii finalnej walidacji podaje durable `0018` udowodnione na writable handle po COMMIT; status i tak wymusza ocenę operatora), `OBS-PR11-03` (domknięcie okna connect→zapis opiera się na semantyce współdzielenia plików wspieranego Windows plus weryfikacji na otwartym uchwycie). Wcześniejsze otwarte findings projektu pozostają bez zmian.
- **Granice po zamknięciu:** orchestrator jest zmergowany i formalnie zamknięty; production migration readiness = `VERIFIED IN MERGED CODE`; produkcyjna baza nadal `0014` (SHA `9906AFBFB580BE8F576A6449B0930C41ED964FED814D99C947D1C28C5B060836`, `364544 B`, integrity `ok`, FK `0`, bez sidecarów); rzeczywista migracja `NOT EXECUTED`; owner authorization `NOT GRANTED`. Każda rzeczywista migracja wymaga aktualnego SHA, aktualnego rozmiaru, nowego snapshotu, braku sidecarów, jawnej flagi `--confirm-production-schema-migration-0014-to-0018` i osobnej decyzji właściciela. Drabina `0014→0018` nie jest jedną atomową transakcją; spójne szczeble pośrednie `0015–0017` wymagają jawnego resume; finalna awaria może wymagać oceny operatora; snapshot nie jest automatycznie przywracany. Controlled-live pozostaje `NOT READY`; realny Fetch i staged A1/A2/B pozostają niewykonane; cały Etap 2 nie jest zamknięty.
- **Następny krok:** `NOT STARTED`. Będzie nim rzeczywista migracja `data/agent.db` `0014→0018` i będzie wymagała osobnej jawnej zgody właściciela. Właściciel tej zgody jeszcze nie udzielił; ten ADR jej nie udziela.

### ADR-110: Polimorficzne recovery po finansowym `SETTLED` dla `TOPIC_GENERATION`

- **Data/status:** 2026-07-22; decyzja implementacyjna w jawnie autoryzowanej, zamkniętej fali naprawczej; `CANDIDATE COMPLETE — READY FOR FINAL RE-REVIEW`. Nie jest to approval, merge, migracja produkcji ani zamknięcie Etapu 2.
- **Problem:** po jednym requestcie tematów i trwałym zapisie rzeczywistego usage attempt mógł być `SETTLED`, gdy proces kończył się przed scoringiem/finalizacją. Recovery obsługiwało tylko RESEARCH, walidator cache liczył research-only usage, a trigger `EXECUTION_RECOVERY` wymagał `research_run`; job/run pozostawały nieterminalne i blokowały konto bez legalnego retry.
- **Decyzja:** ten sam append-only dowód `EXECUTION_RECOVERY` obsługuje dwa jawne warianty workflow. Dla `TOPIC_GENERATION` legalny jest wyłącznie stan: job/run/attempt o spójnym lineage, stage `topics`, attempt #1, dokładny request ID, frozen provider/model, zużyta approval, jedno i tylko jedno usage `task='topics'`, zgodny koszt/cache, brak `research_run`, topiców i generated_topics. Maintenance albo publiczny resolver `CHARGED_KNOWN + EXECUTION_FAILED` terminalizuje run/job do `FAILED`, czyści lease, marker i rezerwację, lecz nie zmienia attemptu, usage ani kosztu i nie tworzy requestu #2.
- **Fail-closed:** `NOT_CHARGED`, `CHARGE_UNKNOWN`, inna execution resolution, inny koszt/task/request/provider/model, brak lub wiele usage, obce konto, attempt #2, niewłaściwa approval albo istniejący wynik topiców są odrzucane bez mutacji. Awaria po dopisaniu eventu wycofuje całą transakcję; replay po recovery jest idempotentny.
- **SQLite:** ponieważ `0020` nie została zmergowana ani użyta produkcyjnie, jej istniejące triggery zostały rozszerzone polimorficznie; nie powstała `0021`. Historyczna gałąź RESEARCH zachowuje dotychczasowe predykaty i skutki.
- **Dowód:** 23 nowe przypadki; collect/exact-once/full `1821/1821`, 0 skipped, 0 xfail. Zielone są wcześniejsze F-1, durable provider, RESEARCH settled/reconciliation, fresh/upgrade/rerun/rollback/reopen/integrity/FK migracji `0020`, worker/reaper/maintenance oraz self-review z fake callerem i failpointem.
- **Granice:** zero sieci/API/SDK/browsera/publikacji/kosztu; wszystkie zapisy tylko do temp DB. Produkcyjna baza pozostaje `0019`, SHA `AE8EFCE9A9036BEDD2DDBE2969552B3A97FA8F9A68588AC4BAC4E0562A716944`, bez migracji i bez sidecarów.

### ADR-111: Publiczny controlled-live dla dokładnie jednego istniejącego joba `TOPIC_GENERATION`

- **Data/status:** 2026-07-22; decyzja implementacyjna w wąskiej fali jawnie zleconej przez właściciela. `CANDIDATE COMPLETE — READY FOR INDEPENDENT REVIEW`. Nie jest to approval live, wykonany request, commit/PR ani zamknięcie Etapu 2.
- **Problem:** zmergowany durable lifecycle potrafił targetować job wewnętrznie, lecz publiczny `worker --once` wybierał queue-wide. Historyczny preflight musiał zakończyć się `BLOCKED` przed requestem, bo nie dało się udowodnić „exact approved job, exact one request”.
- **Decyzja:** jedynym publicznym rootem tej operacji jest `controlled-live-topic-generation`. Nie tworzy joba ani approvalu. Wymaga jawnego job/account oraz pełnego preimage bindingu approval (fingerprint, model, max tokens, cap, candidate count), oczekiwanego DB SHA/schema i Git branch/HEAD oraz flagi confirmation. Każdy brak albo drift kończy się przed otwarciem policy gates.
- **Target i konkurencja:** dokładnie jedna iteracja `Worker(target_job_id=job_id)` używa `claim_specific_job`, nigdy `claim_next_job`. Dozwolony jest tylko pristine `TOPIC_GENERATION` (`QUEUED`, attempts 0, `max_attempts=1`, brak run/topic/lease/attempt/usage/markera). Inny claimowalny paid job, aktywny topic job/attempt, verification/reconciliation, niewykorzystana paid approval albo aktywny runtime powodują odmowę bez mutacji konkurencyjnego stanu.
- **Policy/recovery:** istniejący bezpieczny profil controlled-live jest otwierany procesowo na jedną iterację po zapisaniu dokładnego snapshotu pięciu flag; browser pozostaje wyłączony. Trwały marker sesji i `try/finally` przywracają snapshot również po `BaseException`; recovery przy restarcie przywraca wyłącznie flagi i nie uruchamia Workera, maintenance ani retry. Błąd restore ma osobny exit i pozostawia marker do oceny operatora.
- **Po requestcie:** terminalny rezultat jest raportowany po read-only reopen. Stan niejednoznaczny nie prowadzi do ponownego providera; istniejący durable reconciliation/recovery contract pozostaje źródłem prawdy. Request ID pozostaje `<job_id>:topics:1`, attempt #2 jest niedozwolony przez lifecycle, a publiczny root wykonuje tylko jedno `run_once`.
- **Kody wyjścia:** `0` terminalny sukces, `1` terminalna porażka, `2` preflight refusal, `3` reconciliation, `4` restore failure, `5` recovery barrier failure.
- **Weryfikacja:** 33 nowe przypadki publicznego entrypointu; collect/full `1854/1854`, 0 skipped/xfail. Wyłącznie fake callery i nowe temp DB; zero realnego API/SDK/sieci/browsera/publikacji/kosztu. Produkcja pozostaje niezmienna na schema 0020.
- **Granice:** generator, scoring, deduplikacja, provider, pricing, migracja 0020, durable lifecycle i istniejący research controlled-live nie zostały rozszerzone poza zależności konieczne do rootu. Realny controlled-live wymaga osobnej decyzji właściciela po niezależnym review.

### ADR-112: Jednorazowy targetowany controlled-live `TOPIC_GENERATION` na zmergowanym `main`

- **Data/status:** 2026-07-23; decyzję i dokładne granice podał człowiek. Wynik wykonania: `SUCCESS`. Decyzja została zużyta i nie autoryzuje kolejnego requestu.
- **Autoryzowany kontrakt:** dokładnie jeden `TOPIC_GENERATION` dla `nothing_is_accidental` na `main` `a4e314ff1d9a9ac6bd24fdcdf159ca3e24356916`; `claude-sonnet-5`; `candidate_count=2`; `max_tokens=1500`; timeout `60 s`; `max_retries=0`; `max_web_searches=0`; cap `0.024303 USD`; approval ważny nie dłużej niż 15 minut; bez browsera, Fetch, publikacji i retry.
- **Przygotowanie:** operacja `controlled-live-topic-generation-20260723-0d2e918e406e`; job `topic-generation-037eb2d3db158a70791e30064ad95403`; intent fingerprint `019b102275c924ef936d173450c21933206206ede95deeda8dd21a52f1124c1d`; pricing fingerprint `1b98c7c9656c5b7791ac4f8eb189d538386c31f52b760920a3f2d89f78bb4062`. Approval `1` był ważny 13:59.723 i został zużyty po 56.898 s.
- **Dowód przed requestem:** branch/HEAD i clean tree zgodne; quiescence `PASS`; pięć flag fail-closed; brak aktywnych jobów, attemptów, reconciliation, approvali i sidecarów; schema `0020`, integrity `ok`, FK `0`; pełna suita i exact collect `1854/1854`; pesymistyczna estymata równa capowi `0.024303 USD`.
- **Trwały wynik:** request `topic-generation-037eb2d3db158a70791e30064ad95403:topics:1`, run `4cf8c448-5358-43c6-9d47-e5daf6d0f040`; dokładnie jeden HTTP 200; attempt #1 `SETTLED`; job `DONE`; run `SUCCESS`; usage count `1`; 219 input, 1269 output, cache `0/0`, web search `0`; actual cost `0.013128 USD`; dwa generated topics; selected topic `21`; reconciliation false.
- **Restore i granice:** report potwierdza `provider_retry_performed=false`, `maintenance_performed=false` oraz identyczny fail-closed snapshot flag przed i po; browser pozostawał wyłączony. Nie wykonano Fetch, publikacji, attemptu #2 ani kolejnego requestu.
- **Stan po:** DB SHA-256 `91f593923f3ca36cd8d2c816219e551b0ff231bc3149d6d584b025465f656a1f`, `700416 B`, schema 0020, 20 migracji, integrity `ok`, FK `0`. Pozostały pusty WAL `0 B` i SHM `32768 B`; quiescence `PASS`, locków brak, sidecarów nie usuwano.
- **Granica statusu:** pojedynczy kontrolowany sukces potwierdza ten konkretny execution path. Nie zamyka Etapu 2, nie potwierdza LEVEL_3 i nie autoryzuje kolejnego live, browsera ani publikacji.

### ADR-113: Formal closure of Stage 2 after independently approved controlled-live `TOPIC_GENERATION`

- **Data/status/decyzja:** 2026-07-23; koordynator projektu formalnie zatwierdził **`ETAP 2 — CLOSED`** po niezależnym post-live review `APPROVE WITH MINOR/P2 — ETAP 2 MAY BE FORMALLY CLOSED`.
- **Zmergowana podstawa:** merge commit `main` `a4e314ff1d9a9ac6bd24fdcdf159ca3e24356916`; durable `TOPIC_GENERATION`, targeted publiczny entrypoint i schema `0020_topic_generation_lifecycle` były na `main` przed live.
- **Tożsamość dowodu:** job `topic-generation-037eb2d3db158a70791e30064ad95403`; request `topic-generation-037eb2d3db158a70791e30064ad95403:topics:1`; run `4cf8c448-5358-43c6-9d47-e5daf6d0f040`.
- **Wynik live:** dokładnie jeden HTTP 200, attempt #1 `SETTLED`, job `DONE`, run `SUCCESS`, approval consumed, usage count `1`, 219 input / 1269 output, dwa generated topics, selected topic `21`, retry `0`, browser `0`, Fetch `0`, research `0`, publikacja `0`, reconciliation `false`. Actual cost `0.013128 USD` przy capie `0.024303 USD`; flagi wróciły do fail-closed.
- **Wynik niezależnego review:** collect `1854`, unique node IDs `1854`, pełna suita `1854/1854`, integrity `ok`, FK `0`, marker usunięty i policy gates fail-closed; review nie znalazł blockera do zamknięcia Etapu 2.
- **Formalne granice:** durable `TOPIC_GENERATION` jest zmergowany i live verified, ale L1 pozostaje `ACTIVE`, **`LEVEL_3 = NOT CONFIRMED`**, a publikacja pozostaje `NOT VERIFIED`. Jeden live nie dowodzi wszystkich przyszłych scenariuszy ani autonomicznego działania całego agenta.
- **Dalsza autoryzacja:** Etap 3 może zostać rozpoczęty wyłącznie osobną decyzją właściciela. Każde kolejne live, realny Fetch/API lub publikacja wymaga osobnej, jawnej zgody właściciela; ADR-113 nie jest approvalem wielokrotnego użytku.
- **Pozostające P2, nieblokujące zamknięcia:** proceduralne puste sidecary po live (`WAL 0 B`, `SHM 32768 B`, bez aktywnych locków); minimalny zakres JSON raportu runtime; historyczny ledger legacy poza tym konkretnym runem. P2 pozostają w backlogu i nie zmieniają statusu `ETAP 2 — CLOSED`.

### ADR-114: Durable content foundation jako osobna WAVE C1 przed generowaniem

- **Data/status historyczny:** 2026-07-23; decyzja implementacyjna pierwszego kandydata, później ocenionego przez niezależny review jako `REJECT — MAJOR`. Bieżący status i jedyną falę naprawczą opisuje ADR-115. ADR-114 nie był approvalem, zamknięciem C1/Etapu 3, migracją produkcji ani zgodą na call.
- **Problem:** historyczna `content_items` z `0001` była tylko szkieletem bez zamkniętego lifecycle, modeli, job/run relation, frozen input, evidence lineage, durable intentu i recovery. Aktualny ledger kończył się na `0020`; historyczne wskazanie migracji `0011` było nieaktualne.
- **Decyzja danych:** `0021_durable_content_foundation` przebudowuje zamknięty CHECK `jobs.kind` i zachowuje legacy surface `content_items`, dodając C1 identity oraz tabele `content_frozen_inputs`, `content_evidence_items`, `content_runs`, `content_article_briefs`, `content_call_intents`, `content_provider_attempts` i `evaluations`. Schema+ledger są jednym COMMIT-em; runner przywraca PRAGMA także po rollbacku.
- **Snapshot i lineage:** tylko account-scoped Research Card z rekomendacją PROCEED i kompletnym exact claim→VERIFIED source→evidence excerpt→OK retrieval może utworzyć content. Kanoniczne JSON preimages, ich hashe i znormalizowane rows są append-only; każda późniejsza różnica karty, źródła, excerptu, retrieval ID/URL/fingerprint kończy się fail-closed przed startem lub mutacją lifecycle.
- **Lifecycle:** `DRAFT→PREPARED`; `PREPARED→RUNNING|SKIPPED|FAILED|NEEDS_VERIFICATION`; `RUNNING→PREPARED|REVISE|SKIPPED|PENDING_APPROVAL|FAILED|NEEDS_VERIFICATION`; `REVISE→PREPARED|RUNNING|SKIPPED|FAILED|NEEDS_VERIFICATION`. Terminalne stany C1 nie mają wyjść; identyczny replay jest no-op, konflikt jest odmową.
- **Job/run/recovery:** `CONTENT` wykorzystuje istniejące jobs/runs, lecz zwykły queue-wide claim go pomija. Run powstaje i wiąże się atomowo pod lease fence. Zero-cost/no-attempt expiry może wrócić do PREPARED/QUEUED poniżej capu; jakikolwiek możliwy external/paid boundary prowadzi do NEEDS_VERIFICATION bez retry.
- **Future intent:** immutable preimage wiąże job/run/content/account/card/type, frozen input i Brief hash, evidence manifest, provider/model/pricing fingerprint, token limits, `web_searches=0`, `max_retries=0`, `attempt_no=1`, wersje, cap i expiry. C1 nie tworzy approvalu ani attemptu; provider jest blokowany aplikacyjnie i SQL.
- **Kompatybilność:** historyczny row `content_items.status='DRAFT'` może pozostać bez C1 identity, aby upgrade nie przepisywał danych. C1 API nigdy nie emituje takiego trwałego stanu po COMMIT; to jawna granica kompatybilności `C1-P2-01`, nie ścieżka wykonawcza.
- **Dowód:** 39/39 C1, collect/unique/full `1893/1893`, 0 duplikatów, pełna negatywna macierz FSM i faultpointy; compile/diff PASS. Produkcja pozostaje na `0020`, koszt `0.000000 USD`, zero skutków zewnętrznych.

### ADR-115: Jedyna fala naprawcza po `C1 REJECT — MAJOR`

- **Data/status/autor decyzji:** 2026-07-23; człowiek autoryzował dokładnie jedną falę naprawczą pięciu findings z niezależnego review, a implementer dobrał minimalne konstrukcje techniczne w tych granicach. Wynik implementera: `C1 REPAIR CANDIDATE COMPLETE — AWAITING INDEPENDENT RE-REVIEW`. C1 nie jest zatwierdzone, Etap 3 pozostaje `IN PROGRESS`, migracja produkcyjna `0021` i controlled-live nie zostały wykonane.
- **Wynik review pierwszego kandydata:** `REJECT — MAJOR`; `MAJOR-01` — manifest wybierał lineage przez tekst/URL zamiast trwałych kluczy; `MAJOR-02` — generyczne terminalizatory i raw `content_runs` mogły rozdzielić cztery lifecycle rows; `MAJOR-03` — content extension nie wymuszało pełnego 1:1 z kanonicznym `provider_attempts`. `MINOR-01` dotyczył braku pełnego porównania ARTICLE/NOTE, a `MINOR-02` przedwczesnej deklaracji braku blockerów.
- **Decyzja lineage:** 0021 rozszerza istniejące `evidence_source_lineage` o jawne `confirmed_claim_id/ordinal`, research job/topic i fingerprint. Stary niejawny row zachowuje NULL jako historię i nie jest dopuszczany do content snapshotu; migracja nie fabrykuje identity przez tekst, URL ani kolejność. Frozen evidence przechowuje pełne durable IDs i fingerprint, a start wykonania ponawia walidację w tej samej transakcji tuż przed attach.
- **Decyzja lifecycle:** `jobs.execution_generation` rośnie przy każdym claimie CONTENT i jest obowiązkowym fence w `JobExecutionContext`. Jedyną granicą przejść jest append-only `content_transition_commands`: SQL weryfikuje konto, workflow, relacje, ownera, lease, generation i payload, po czym jeden trigger aktualizuje dokładnie content_run, zsynchronizowany content_item oraz — dla terminala — dokładnie run i job. Generyczne terminalizatory job/run, cancel i nieogrodzone mutacje runtime odmawiają CONTENT.
- **Decyzja provider ledger:** `provider_attempts` pozostaje jedynym ledgerem statusu, usage, kosztu i reconciliation. `content_provider_attempts` jest append-only rozszerzeniem 1:1, którego request identity, job/run/content/account/stage/attempt/provider/model muszą zgadzać się z call intentem i parentem. Parent+extension powstają w jednej transakcji; parent bez extension jest blokowany triggerem, extension bez parenta deferred FK, a `model_usage` nadal wskazuje wyłącznie kanoniczny parent. Nie dodano realnego callera.
- **Decyzja workflow:** każda mutacja content runtime porównuje execution workflow z `content_items.type`, `jobs.workflow`, `runs.workflow` i `content_runs.workflow`; ARTICLE↔NOTE mismatch odmawia przed zapisem.
- **Dowód kandydacki:** 25/25 nazwanych kontrprób; 69/69 content+migration; collect 1923, 1922 różne tekstowo node IDs i jedna zastana kolizja param ID poza C1; pełna suita 1923/1923. Wszystkie testy używają nowych temp SQLite i fake/local paths. Produkcja pozostaje `0020`, SHA `91f593923f3ca36cd8d2c816219e551b0ff231bc3149d6d584b025465f656a1f`, `700416 B`; zero API/sieci/browsera/publikacji i koszt `0.000000 USD`.
- **Granice:** findings są zaadresowane wyłącznie w kandydacie i pozostają formalnie otwarte do niezależnego re-review. C2, planner, writer, Article Brief generation, artykuły/Notes, audyty, rewrite, approval L1, profile cenowe i realny provider pozostają `NOT STARTED`. Nie wykonano stage, commita, pushu, PR ani merge.

### ADR-116: C2 jako fake-only, resumowalny pipeline z logicznym routingiem i dokładnie jedną poprawką

- **Data/status:** 2026-07-23; `C2 CANDIDATE COMPLETE — AWAITING INDEPENDENT REVIEW`. Decyzja implementacyjna w zakresie jawnie zleconym przez właściciela; nie jest approvalem, merge ani zgodą na C3–C5.
- **Granica wykonawcza:** C2 ma tylko lokalny, targeted entrypoint i `OFFLINE_PIPELINE`. Zwykły worker nadal nie wybiera CONTENT z kolejki. Fake caller jest jedyną osiągalną implementacją `WriterPort`; realny resolver zawsze odmawia. Brak SDK, sieci, browsera, publikacji i approval UI.
- **Plan i brief:** `content_plans` przechowuje wersjonowany, hash-addressed plan związany z account/content/run/Research Card/frozen input/evidence/type/route config. Istniejący `content_article_briefs` przechowuje także typowany Note Brief. Planner przyjmuje wyłącznie `PROCEED` i kompletne frozen evidence; odrzuca drift identity/workflow/topic/account oraz AI/budowę projektu dla głównej marki; nie tworzy faktów ani URLs.
- **Routing:** ARTICLE wymaga `FABLE_5_ARTICLE`, NOTE `SONNET_5_NOTE`. To logiczne decyzje właściciela, nie techniczne identyfikatory API. Provider, API model ID, availability i pricing mają status `UNVERIFIED`; fallback ma status `FORBIDDEN`. Każda zmiana wymaga nowej wersji `config/content_routing.yaml`.
- **Prywatny styl:** raw source pozostaje gitignored, nie trafia do repo, logów ani runtime. C2 czyta tylko wersjonowane profile pochodne. Ponieważ lokalny korpus nie ma wiarygodnego zestawu Notes, profil Notes jest jawnie `PROVISIONAL`.
- **Writer i ledger:** każdy z maksymalnie dwóch intentów ma `call_mode=FAKE`, `max_cost_usd=0`, exact route i fingerprint. Każda próba dostaje osobny canonical `provider_attempts` parent oraz ścisłe `content_writer_attempts` extension; `model_usage` ma `dry_run=1` i koszt/actual cost `0.000000`. Historyczny CHECK canonical ledgeru wymaga dodatniej rezerwacji, dlatego fake parent używa wyłącznie strukturalnego floor `0.000001`, który nie jest wydatkiem ani job budget hold i zostaje rozliczony do actual cost zero.
- **Draft/evaluations/rewrite:** drafty są append-only i związane z settled fake attemptem. Dokładnie dziewięć trwałych evaluations obejmuje evidence coverage, unsupported claims, brand/topic, style, negative style, typ/długość, fake personal experience, title/hook i brief compliance. Unsupported claim, fake personal experience i brand conflict dają `BLOCK`; naprawialne braki dają `REWRITE_ONCE`. Próba #2 jest dopuszczona wyłącznie po kompletnym durable rewrite decision; próba #3 jest niedopuszczalna.
- **Terminalizacja/recovery:** sukces ustawia title/body i używa tej samej append-only command boundary C1 do atomowego `PENDING_APPROVAL`/`SUCCESS`/`DONE`. Recovery uznaje wyłącznie spójne fake attempts/zero-cost dry usage za bezpieczne checkpointy; external-effect marker lub niekanoniczny attempt nadal prowadzi do `NEEDS_VERIFICATION`. Pokryto restart po briefie, rozpoczęciu próby, draftcie oraz między pierwszym draftem i poprawką, takeover, stary fence i dwa workery.
- **Migracja:** `0022_offline_content_pipeline` jest addytywna względem zamkniętego `0021`; jawny `scripts/migrate_schema_0022.py` wymaga `--confirm-0021-to-0022`. Wszystkie migracje C2 wykonano tylko na nowych/temp bazach. Produkcja pozostaje na `0020`; C4 nie rozpoczęto.
- **Dowód:** C2 `22/22`; pełna suita `1945/1945`; collect i exact case-sensitive unique node IDs `1945`, duplikaty `0`, zero skipped/xfail. `compileall app scripts` i `git diff --check` są wymagane w finalnym checkpointcie. Koszt `0.000000 USD`.
- **Pozostające P2/gates:** `C2-P2-01` — strukturalny positive reservation floor starego ledgeru mimo actual/usage zero; `C2-P2-02` — targeted fake CONTENT działa w krótkim bounded lease bez generic heartbeat, bo C1 słusznie blokuje nieogrodzony heartbeat; przed realnym/duższym writerem C5 potrzebny jest dedykowany fenced content heartbeat; `C2-P2-03` — Notes profile pozostaje provisional; `C2-P2-04` — techniczne route IDs/availability/pricing muszą zostać osobno potwierdzone przed C5. Żaden z tych punktów nie otwiera realnego callera w C2.

### ADR-117: Formalne zamknięcie WAVE C2 po niezależnym review, merge PR #22 i post-merge checkpointcie

- **Data/status/decyzja:** 2026-07-23; ACCEPTED — formalna decyzja właściciela. C2 = `CLOSED — MERGED AND POST-MERGE VERIFIED`; cały Etap 3 pozostaje `IN PROGRESS`, a C3–C5 pozostają `NOT STARTED`.
- **Assurance i merge:** pełny niezależny review oraz wąski re-review korekty whitespace wydały `APPROVE WITH MINOR/P2`. Commit C2 `86e284e997607b54107cbb42deb35410a38bd0e4` (`feat: add offline content pipeline for stage 3`) został zmergowany przez PR #22 `Stage 3 C2: offline content pipeline` jako merge commit `458a86e9d86c9f09fd1f8ec17d3e8134aa44155a`.
- **Post-merge checkpoint:** C2 `22/22`, zatwierdzona regresja `384/384`, pełna suita `1945/1945`, zero failures/skipped/xfail/errors; collect i exact case-sensitive unique node IDs `1945`, exact duplicates `0`; `compileall app scripts` i `git diff --check` PASS.
- **Granica techniczna:** zamknięte C2 pozostaje wyłącznie offline/fake i kończy w `PENDING_APPROVAL`. ARTICLE używa logicznego route key `FABLE_5_ARTICLE`, NOTE `SONNET_5_NOTE`, fallback jest `FORBIDDEN`; provider, techniczne API model IDs, availability i pricing pozostają `UNVERIFIED`, a Notes profile `PROVISIONAL`. Nie istnieje approval UI ani osiągalny realny content provider.
- **Produkcja i skutki:** kod na `main` zawiera migracje `0021` i `0022`, lecz produkcyjna baza pozostaje byte-identical na `0020_topic_generation_lifecycle`, a runtime pozostaje wobec niej fail-closed. Nie wykonano realnego API/SDK, prawdziwego artykułu, controlled-live, browsera, publikacji ani migracji produkcji; koszt `0.000000 USD`.
- **Pozostające P2, nieblokujące zamknięcia:** techniczny `reserved_amount_usd = 0.000001` zawsze rozliczany do zera; globalny brand-topic block poprawny dla jednego konta; trzy evaluations zależne od self-report fake draftu; brak dedykowanego CONTENT heartbeat dla przyszłego realnego writera; historyczne warstwy dokumentacji; Notes profile `PROVISIONAL`; provider/API IDs, availability i pricing `UNVERIFIED`. Punkty pozostają otwarte bez nowej fali i nie autoryzują C3–C5 ani działania zewnętrznego.

### ADR-118: C3 jako provider-ready, lecz nadal offline i fail-closed

- **Data/status/decyzja:** 2026-07-23; decyzja implementacyjna w zakresie autoryzowanym przez właściciela. Status (pierwotny, 2026-07-23): `C3 CANDIDATE COMPLETE — AWAITING INDEPENDENT REVIEW` — zaktualizowany po review i merge, patrz **Aktualizacja post-merge** niżej. To nie jest approval, zamknięcie, commit/merge, produkcyjna migracja ani zgoda na realny request.
- **Kontrakt:** `WriterRequest` łączy zamrożone lineage, Research Card i dozwolone evidence, trwały plan/brief, wersjonowane profile pochodne, policy, route i limity. Odpowiedź jest zamkniętym `WriterSuccess|WriterFailure` z draftem albo typowanym failure, providerem, route key, technicznym modelem, usage, stop reason i provider request ID.
- **Routing/config:** ARTICLE pozostaje `FABLE_5_ARTICLE`, NOTE `SONNET_5_NOTE`, fallback `FORBIDDEN`. Logical route nie jest API model ID. Resolver oddziela provider, model, pricing i availability/configuration; `UNVERIFIED` odmawia przed SDK. Aktualnych cen i dostępności nie deklarowano.
- **Adapter:** SDK jest tworzone leniwie dopiero po pełnym preflightcie; caller i factory są wstrzykiwane; `max_retries=0`; timeout dodatni i nie większy niż 30 s; jedna logiczna próba może wykonać najwyżej jeden call; brak fallbacku i domyślnego modelu produkcyjnego. Wspierany real composition root nadal bezwarunkowo odmawia po sprawdzeniu konfiguracji; C3 storage odrzuca `CONTROLLED_PROVIDER`.
- **Prompt/private style:** prompt ma kanoniczny fingerprint i składa tylko dozwolone trwałe dane oraz profile pochodne. Zakazuje fikcyjnej autobiografii, podróży, rodziny, rozmów i wspomnień; pierwsza osoba służy wyłącznie opinii/rozumowaniu. Raw private style source nie jest wejściem runtime, logu ani durable resultu.
- **Ledger/recovery:** migracja `0023_provider_ready_writer` rebuilds intent/attempt z zachowaniem C2 rows, dodaje dokładne metadata oraz append-only `content_writer_results`. Intent poprzedza caller; attempt przechodzi przez `REQUEST_STARTED`; usage i settlement są zapisywane także przy parse/truncation/schema po zwróceniu usage. Wynik utrwalony przed settlementem jest resumowalny bez drugiego calla; wynik callera nieutrwalony wymaga `NEEDS_RECONCILIATION/NEEDS_VERIFICATION`; podwójny settlement, stary fence, drugi worker i attempt #3 są blokowane.
- **Heartbeat:** dedykowanego CONTENT heartbeat nie dodano. W C3 provider-ready timeout jest ograniczony do 30 s, testowy lease do 60 s, a realny composition root nie istnieje, więc nie powstał aktualny blocker. Przed C5 lub podniesieniem timeoutu ponad bezpieczny lease decyzja musi być ponowiona; brak heartbeat pozostaje gate/P2, nie pretekstem do rozszerzenia C3.
- **Migracja:** jawny `scripts/migrate_schema_0023.py` wymaga potwierdzenia `0022→0023`; testy świeżej drabiny, idempotencji i failpointu dowodzą atomowego rollbacku schema+ledger. Wszystkie zapisy były na nowych temp DB. Produkcji na `0020` nie migrowano.
- **Dowód implementera:** C3 `26/26`; celowana regresja C1/C2/provider/storage `463/463`; pełna suita `1971/1971`; collect/exact case-sensitive unique `1971`, exact duplicates `0`, skipped `0`, xfail `0`, errors `0`; `compileall` i `git diff --check` PASS; koszt `0.000000 USD`.
- **Otwarte gates/P2:** brak niezależnego review; provider/model/pricing/availability nadal wymagają późniejszego potwierdzenia; Notes profile pozostaje `PROVISIONAL`; strukturalny offline reservation floor starego ledgera pozostaje rozliczany do zera; heartbeat wymaga ponownej oceny przed realnym/dłuższym callem. C4 i C5 są `NOT STARTED`.
- **Aktualizacja post-merge (2026-07-24):** pierwotny status wyżej opisuje 2026-07-23. Kandydat przeszedł pełny niezależny review `APPROVE WITH MINOR/P2`, poprawkę MINOR podnoszącą C2+C3 z `47/47` na `48/48` oraz wąski re-review `APPROVE`; następnie commit implementacji `a3096355ab0a648805099f2bfd30ab5d87600fbc` został zmergowany PR #24 jako merge commit `81936476ad4425e959730f4425c979f1671ef4f1` (rodzice `54c761d0d6c1b7b7402d89e9c24db169b694f00a` i `a3096355ab0a648805099f2bfd30ab5d87600fbc`), a `main` zsynchronizowano ff-only. Post-merge: C3 `26/26`, C2+C3 `48/48`, regresja `463/463`, pełna suita `1971/1971`, collect/exact unique `1971`, duplikaty/skipped/xfail/errors `0`, `compileall`/`git diff --check` PASS; koszt `0.000000 USD`; produkcja nadal `0020` (kod `0023`), bez migracji/API/artykułu/publikacji. Nowy status: `C3 CODE MERGED — POST-MERGE VERIFIED — AWAITING DOCUMENTATION SYNC MERGE`. Osiem P2 pozostaje otwartych (paid usage/terminalizacja kosztu >0; CONTENT heartbeat/lease; evaluations zależne od self-report; sentinel migracji 0023; obsługa SQLite URI w helperze migracyjnym; flake heartbeat pod obciążeniem; provider/model/pricing/availability `UNVERIFIED`; Notes profile `PROVISIONAL`), a P2-1/P2-2/P2-3 pozostają wymaganymi gate'ami przed C5. Formalne `C3 FULLY CLOSED` nastąpi dopiero po niezależnym review i merge dokumentacyjnego PR; C4/C5 pozostają `NOT STARTED`.
- **Aktualizacja stanu właściciela (2026-07-24):** autorytatywny stan wejściowy WAVE C4 potwierdza, że wymagany review i merge dokumentacji C3 nastąpiły; bieżący status C3 to `C3 FULLY CLOSED — DOCUMENTATION MERGED AND VERIFIED`. Ta korekta nie zmienia historycznych statusów kandydata ani pre-merge.

### ADR-119: C4 jako trwała, offline i provider-independent decyzja gotowej treści

- **Data/status/autor decyzji:** 2026-07-24; właściciel autoryzował wyłącznie implementację WAVE C4 i rozstrzygnął wcześniejszą roadmapową etykietę C4: C4 jest warstwą decyzji, a migracja produkcji pozostaje osobną, nieautoryzowaną operacją. Wynik implementera: `C4 CANDIDATE COMPLETE — AWAITING INDEPENDENT REVIEW`; C4 nie jest zatwierdzone ani zamknięte.
- **Granica Policy Engine:** jedynym miejscem tworzenia decyzji jest `PolicyEngine.decide_content`. Wejście pochodzi wyłącznie z trwałego, aktualnego snapshotu: `content_item`, current draft i jego writer/attempt/usage lineage, dziewięć evaluations, typ ARTICLE/NOTE, account mode/active/autonomy, per-type approval policy, evaluator version, policy version i threshold. Canonical JSON ma SHA-256 input fingerprint; wynik ma osobny decision fingerprint i jednoznaczny reason code.
- **Macierz:** LEVEL_0 i LEVEL_1 zawsze wymagają człowieka dla treści przechodzącej evaluations; LEVEL_2 wymaga człowieka dla ARTICLE, a może autonomicznie zdecydować NOTE; LEVEL_3 może autonomicznie zdecydować ARTICLE i NOTE. `DRAFT_ONLY` oraz per-type `require_*_approval` wymuszają człowieka; `COMMENT_ONLY`/`RESEARCH_ONLY`, inactive account, wynik poniżej progu i evaluation FAIL prowadzą do odrzucenia. Twarde typy `UNSUPPORTED_CLAIMS`, `BRAND_TOPIC_POLICY` i `FAKE_PERSONAL_EXPERIENCE` zawsze wygrywają ze score.
- **Fail-closed:** missing/duplicate/unknown/contradictory evaluation, zła wersja, złe draft binding, zastąpiony draft, unsupported content/autonomy, terminalny content albo niespójny lineage dają stale/verification, nigdy approve. Niepoprawna wersja policy lub threshold daje odrębny technical failure. Oczekiwany fingerprint jest sprawdzany ponownie po odczycie pod transakcją.
- **Trwałość/lifecycle:** `0024_autonomous_content_decision` dodaje append-only `autonomous_decisions`. Każdy row wiąże exact content/draft/job/run/account, autonomy/mode, actor, outcome/lifecycle, reason, policy/threshold/score, canonical input i decision JSON, oba hashe, approval ID, lease owner, generation fence i timestamp. Terminalny outcome jest unikalny per content. Human-required tworzy `approvals.PENDING` i pozostawia `content_items=PENDING_APPROVAL`; autonomous approve/reject kończy kanoniczny `content_items` jako `APPROVED|REJECTED`; stale/technical kończą jako `NEEDS_VERIFICATION|FAILED`. `content_runs=PENDING_APPROVAL` zachowuje terminalny handoff wykonania C3, nie jest drugim stanem decyzji.
- **Atomowość/recovery:** `BEGIN IMMEDIATE`, aktualny execution fence, dwukrotna deterministyczna rewalidacja, append-only audit, opcjonalny approval, istniejąca C1 transition command i applied flip są jednym commitem. Błąd po apply rollbackuje audit, approval i lifecycle. Identyczny replay zwraca istniejący wynik; sprzeczny replay odmawia; dwa workery dają jeden terminalny audit; stary owner/fence nie zapisuje.
- **Provider/koszt:** C4 API nie przyjmuje writera, SDK ani callera. Decyzja działa wyłącznie na utrwalonych artefaktach C3; test porównuje attempts, usage, settlement i koszt przed/po granicy C4. Wynik: zero nowych prób/usage/settlementu/kosztu i `0.000000 USD`.
- **Migracja:** forward-only `0024`; jawny `scripts/migrate_schema_0024.py` wymaga `--confirm-0023-to-0024`; fresh DB, exact upgrade 0023, idempotent rerun, raw SQLite, FK i concurrency są testowane. Nie ma backfillu fałszywych decyzji. Produkcja pozostaje na `0020`.
- **Composition root:** wspierany flow pozostaje `app.main`/Worker → `JobDispatcher` → `run_offline_content_pipeline` → C4 PolicyEngine → `SqliteStorage.finalize_content_decision` → lifecycle/job terminalization. Nie powstał realny provider root ani publikacja.
- **Dowód implementera:** C4 `23/23`; C2 `22/22`; C3 `26/26`; pełna suita `1994/1994`; collect/unique case-sensitive `1994`, duplikaty `0`, przyrost `+23` wobec 1971; skipped/xfail/failures/errors `0`; compile/diff wymagane w finalnym checkpointcie.
- **Zachowane granice:** C5 i PRE-C5 QUALITY GATE pozostają `NOT STARTED`; prywatnego korpusu nie otwierano. Osiem P2 pozostaje otwartych, a paid usage >0, CONTENT heartbeat/lease i evaluation self-report nadal są gate'ami C5. Zero API/SDK/sieci/browsera/publikacji/controlled-live, zero migracji produkcji i zero stage/commita/pushu/PR/merge.
