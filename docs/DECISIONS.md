...
- **Data:** 2026-07-13
- **Status:** ACCEPTED / wdrożone i zweryfikowane offline; końcowa akceptacja restartu pozostaje NOT_STARTED.
...
- **Trwałość i reason:** wyłącznie `ScheduledJobEnqueuer` przekształca decyzję w nowy `Job`, zapisując `earliest_run_at` w UTC i `schedule_reason` ze zamkniętego, krótkiego zbioru kodów. Niskopoziomowe `enqueue_job` waliduje kod i odmawia pustej, wieloliniowej albo dowolnej wartości. Idempotency obejmuje oba pola, więc powtórzenie tego samego harmonogramu jest no-opem, a odmienna decyzja konfliktem.
...
- **Statusy:** polityka strefy UTC/IANA/DST, centralny enqueue, kontrolowany reason oraz claim eligibility = VERIFIED OFFLINE; końcowa akceptacja restartu i usługa schedulera systemowego = NOT_STARTED; paid worker, browser/public worker i realne resume pozostają BLOCKED/NOT IMPLEMENTED.
- **Weryfikacja:** `tests/test_scheduling.py` — **31 passed** (okna, weekend, requested time, IANA/DST, błędna konfiguracja, UTC/reopen, reason, future eligibility, idempotency i dwa połączenia SQLite z `Barrier`); pełny suite **623 test cases passed**, `data/agent.db` bez zmiany, brak API/sieci/researchu, koszt 0 USD.
- **Kto podjął:** właściciel zatwierdził ograniczony offline scope; wykonanie: Codex.
