"""Explicit 0025 -> 0026 migrator; never imported by runtime roots."""
...
from app.storage.db import ExplicitMigrationError, migrate_0025_to_0026
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from exact schema 0025 to exact schema 0026."
        )
    )
    parser.add_argument("--db-path", required=True, type=Path)
    parser.add_argument(
        "--confirm-0025-to-0026",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_0025_to_0026:
        print(
            "SCHEMA MIGRATION: failed closed: --confirm-0025-to-0026 is required.",
            file=sys.stderr,
        )
        return 2
    try:
        result = migrate_0025_to_0026(args.db_path)
