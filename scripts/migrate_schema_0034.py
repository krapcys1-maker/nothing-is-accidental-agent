"""Explicit 0033 -> 0034 migrator; never imported by runtime roots."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "0033 to exact schema 0034."
        )
    )
    parser.add_argument("--db-path", required=True, type=Path)
    parser.add_argument(
        "--confirm-0033-to-0034",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_0033_to_0034:
        print(
            "SCHEMA MIGRATION: failed closed: --confirm-0033-to-0034 is required.",
            file=sys.stderr,
        )
        return 2
    try:
        result = migrate_0033_to_0034(args.db_path)
