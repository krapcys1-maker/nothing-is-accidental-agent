"""Explicit 0020 -> 0021 migrator; never imported or called by runtime roots."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Explicitly migrate one named SQLite database from exact schema 0020 "
            "to exact schema 0021. This command never starts runtime."
        )
    )
    parser.add_argument("--db-path", type=Path, required=True)
    parser.add_argument(
        "--confirm-0020-to-0021",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_0020_to_0021:
        print(
            "SCHEMA MIGRATION: failed closed: --confirm-0020-to-0021 is required.",
            file=sys.stderr,
        )
        return 2
    try:
        result = migrate_0020_to_0021(args.db_path)
