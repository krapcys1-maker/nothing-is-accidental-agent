"""Explicit 0016 -> 0017 migrator; never imported or called by runtime roots."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Explicitly migrate one named SQLite database from exact schema 0016 "
            "to exact schema 0017. This command never starts runtime."
        )
    )
    parser.add_argument("--db-path", type=Path, required=True)
    parser.add_argument(
        "--confirm-0016-to-0017",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_0016_to_0017:
        print(
            "SCHEMA MIGRATION: failed closed: --confirm-0016-to-0017 is required.",
            file=sys.stderr,
        )
        return 2
    try:
        result = migrate_0016_to_0017(args.db_path)
