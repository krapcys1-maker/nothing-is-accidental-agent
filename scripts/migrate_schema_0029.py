"""Explicit 0028 -> 0029 migrator; never imported by runtime roots."""
...
from app.storage.db import ExplicitMigrationError, migrate_0028_to_0029
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from exact schema 0028 to exact schema 0029."
        )
    )
    parser.add_argument("--db-path", required=True, type=Path)
    parser.add_argument(
        "--confirm-0028-to-0029",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_0028_to_0029:
        print(
            "SCHEMA MIGRATION: failed closed: --confirm-0028-to-0029 is required.",
            file=sys.stderr,
        )
        return 2
    try:
        result = migrate_0028_to_0029(args.db_path)
