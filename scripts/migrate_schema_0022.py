"""Explicit 0021 -> 0022 migrator; never imported or called by runtime roots."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Explicitly migrate one named SQLite database from exact schema 0021 "
            "to exact schema 0022. This command never starts runtime."
        )
    )
    parser.add_argument("--db-path", type=Path, required=True)
    parser.add_argument(
        "--confirm-0021-to-0022",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_0021_to_0022:
        print(
            "SCHEMA MIGRATION: failed closed: --confirm-0021-to-0022 is required.",
            file=sys.stderr,
        )
        return 2
    try:
        result = migrate_0021_to_0022(args.db_path)
