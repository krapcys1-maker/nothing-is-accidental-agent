...
> **2026-08-12 — właściciel skorygował model i czas, nie wynik.** Właściciel jednoznacznie ustalił Opusa dla czterech ról artykułowych, rozszerzył cel do pełnego online E2E i wskazał, że 30 sekund jest operacyjnym błędem — ma być maksymalnie pięć minut. Agent zmienił kontrakt do 300 s i kontynuował, ale nie uznał tego za zgodę na retry, fallback ani publikację. Po dwóch kolejnych zewnętrznych `APIConnectionError` reviewera zatrzymał flow z jawnym `NEEDS_VERIFICATION`, zgodnie z granicami kosztu i bezpieczeństwa.

> **2026-08-12 — właściciel dopuścił narzędzie, nie operację.** Polecenie pozwalało dodać operator path dla dokładnie `0033→0034` i testować go wyłącznie na nowych bazach tymczasowych. Osobno i jednoznacznie zabraniało migracji produkcyjnej, kopii produkcji, sidecarów, sieci, API, publikacji, kosztu oraz operacji Git. Agent zachował tę granicę: powstał kandydat CLI i dowód rollbacku, ale produkcja nadal stoi na `0033` i wymaga nowej decyzji właściciela przed jakimkolwiek zapisem.
