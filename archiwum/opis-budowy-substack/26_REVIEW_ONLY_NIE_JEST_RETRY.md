...
Skupiony re-review ujawnił jeszcze subtelniejszy błąd: samo użycie tego samego `approval_ref` nie wystarcza, jeśli proces przy restarcie tworzy nowe `approved_at` i `expires_at`. Wtedy „resume” technicznie wystawia inną zgodę. Poprawiona droga ładuje dokładny immutable payload z SQLite, nie odnawia ważności i odmawia przy zmianie któregokolwiek elementu kontraktu. Analogicznie model nie dostaje już starego `enabled/budget_tokens`; Opus 5 i Sonnet 5 otrzymują exact adaptive thinking, a głębokość pozostaje osobnym, trwałym effortem.

Po niezależnym zatwierdzeniu poprawki PR #46 został zmergowany, a produkcja przeszła kontrolowany krok `0038→0039` z pełnym backupem i zachowaniem danych. To nadal nie uruchomiło Reviewera: próba samej bramki zatrzymała się przy nierozstrzygniętej ekspozycji `0.738880 USD`, zanim kod dotknął sekretu, SDK lub callera. Migracja przygotowała bezpieczną drogę; nie zamieniła niepewnego efektu zewnętrznego w pozwolenie na retry.
