...
def test_migration_cli_0018_requires_confirmation_and_is_exact(tmp_path, capsys):
    ...
    import scripts.migrate_schema_0029 as migration_cli_0029
    import scripts.migrate_schema_0030 as migration_cli_0030
    import scripts.migrate_schema_0031 as migration_cli_0031
    import scripts.migrate_schema_0032 as migration_cli_0032
    import scripts.migrate_schema_0033 as migration_cli_0033
    ...
    with pytest.raises(SchemaVersionTooOld):
        SqliteStorage.open(path)
    assert migration_cli_0025.main(["--db-path", str(path)]) == 2
    assert migration_cli_0025.main([
        "--db-path", str(path), "--confirm-0024-to-0025",
    ]) == 0
    with pytest.raises(SchemaVersionTooOld):
        SqliteStorage.open(path)
    assert migration_cli_0026.main(["--db-path", str(path)]) == 2
    assert migration_cli_0026.main([
        "--db-path", str(path), "--confirm-0025-to-0026",
    ]) == 0
    with pytest.raises(SchemaVersionTooOld):
        SqliteStorage.open(path)
    assert migration_cli_0026.main([
    ...
    assert "idempotent=true" in capsys.readouterr().out
    assert migration_cli_0027.main(["--db-path", str(path)]) == 2
    ...
    assert "idempotent=true" in capsys.readouterr().out
    assert migration_cli_0029.main(["--db-path", str(path)]) == 2
    assert migration_cli_0029.main([
        "--db-path", str(path), "--confirm-0028-to-0029",
    ]) == 0
    with pytest.raises(SchemaVersionTooOld):
        SqliteStorage.open(path)
    assert migration_cli_0030.main(["--db-path", str(path)]) == 2
    assert migration_cli_0030.main([
        "--db-path", str(path), "--confirm-0029-to-0030",
    ]) == 0
    with pytest.raises(SchemaVersionTooOld):
        SqliteStorage.open(path)
    assert migration_cli_0031.main(["--db-path", str(path)]) == 2
    assert migration_cli_0031.main([
        "--db-path", str(path), "--confirm-0030-to-0031",
    ]) == 0
    with pytest.raises(SchemaVersionTooOld):
        SqliteStorage.open(path)
    assert migration_cli_0032.main(["--db-path", str(path)]) == 2
    assert migration_cli_0032.main([
        "--db-path", str(path), "--confirm-0031-to-0032",
    ]) == 0
    with pytest.raises(SchemaVersionTooOld):
        SqliteStorage.open(path)
    assert migration_cli_0033.main(["--db-path", str(path)]) == 2
    ...
    assert database_schema_versions(path)[-1] == "0042_research_conservative_adjudication"
