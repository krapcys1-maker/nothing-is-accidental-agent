"""Explicit 0022 -> 0023 migrator; never imported by runtime roots."""
...
from app.storage.db import ExplicitMigrationError, migrate_0022_to_0023
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from exact schema 0022 to exact schema 0023."
        )
    )
    parser.add_argument("--db-path", required=True, type=Path)
    parser.add_argument(
        "--confirm-0022-to-0023",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_0022_to_0023:
        print(
            "SCHEMA MIGRATION: failed closed: --confirm-0022-to-0023 is required.",
            file=sys.stderr,
        )
        return 2
    try:
        result = migrate_0022_to_0023(args.db_path)
