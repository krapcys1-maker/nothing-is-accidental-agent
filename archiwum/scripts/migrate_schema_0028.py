"""Explicit 0027 -> 0028 migrator; never imported by runtime roots."""
...
from app.storage.db import ExplicitMigrationError, migrate_0027_to_0028
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from exact schema 0027 to exact schema 0028."
        )
    )
    parser.add_argument("--db-path", required=True, type=Path)
    parser.add_argument(
        "--confirm-0027-to-0028",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_0027_to_0028:
        print(
            "SCHEMA MIGRATION: failed closed: --confirm-0027-to-0028 is required.",
            file=sys.stderr,
        )
        return 2
    try:
        result = migrate_0027_to_0028(args.db_path)
