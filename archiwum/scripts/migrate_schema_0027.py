"""Explicit 0026 -> 0027 migrator; never imported by runtime roots."""
...
from app.storage.db import ExplicitMigrationError, migrate_0026_to_0027
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "from exact schema 0026 to exact schema 0027."
        )
    )
    parser.add_argument("--db-path", required=True, type=Path)
    parser.add_argument(
        "--confirm-0026-to-0027",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_0026_to_0027:
        print(
            "SCHEMA MIGRATION: failed closed: --confirm-0026-to-0027 is required.",
            file=sys.stderr,
        )
        return 2
    try:
        result = migrate_0026_to_0027(args.db_path)
