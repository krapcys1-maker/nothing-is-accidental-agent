"""Explicit 0019 -> 0020 migrator; never imported or called by runtime roots."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Explicitly migrate one named SQLite database from exact schema 0019 "
            "to exact schema 0020. This command never starts runtime."
        )
    )
    parser.add_argument("--db-path", type=Path, required=True)
    parser.add_argument(
        "--confirm-0019-to-0020",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_0019_to_0020:
        print(
            "SCHEMA MIGRATION: failed closed: --confirm-0019-to-0020 is required.",
            file=sys.stderr,
        )
        return 2
    try:
        result = migrate_0019_to_0020(args.db_path)
