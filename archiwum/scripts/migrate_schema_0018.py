"""Explicit 0017 -> 0018 migrator; never imported or called by runtime roots."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Explicitly migrate one named SQLite database from exact schema 0017 "
            "to exact schema 0018. This command never starts runtime."
        )
    )
    parser.add_argument("--db-path", type=Path, required=True)
    parser.add_argument(
        "--confirm-0017-to-0018",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not args.confirm_0017_to_0018:
        print(
            "SCHEMA MIGRATION: failed closed: --confirm-0017-to-0018 is required.",
            file=sys.stderr,
        )
        return 2
    try:
        result = migrate_0017_to_0018(args.db_path)
