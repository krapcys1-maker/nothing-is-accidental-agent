...
- Zbudowane i przetestowane offline: konfiguracja, SQLite z **14 migracjami** (ostatnia `0014_provider_attempt_reconciliation`), Policy Engine, kolejka/worker, ledger provider attempt, durable single-research `durable_provider_v2`, pełny zatwierdzony kontrakt cenowy (`Decimal`), kanoniczny operatorski wrapper `controlled-live-once`, trwałe session/job/request fencing, bezpieczny raport i recovery bez retry, minimalny launcher systemowy, raport read-only i kontrolowany executor migracji. `model_usage` pozostaje jedynym ledgerem kosztu. **1174/1174 testów, wszystkie offline; partycje exact-once 284+284+298+308.**
- WAVE 0A/0B/1A są formalnie **`CLOSED — APPROVED WITH P2`**. Pierwsza WAVE LA-01 została odrzucona jako **`REJECTED — MAJOR`**; naprawa **LA-01-R1 = `APPROVED WITH MINOR/P2 — CHECKPOINTED`**. **LA-02 = `APPROVED WITH MINOR/P2 — CHECKPOINTED`; root cause `PROCESSES_PRESENT` jest `CLOSED`.** Provider request nie został wykonany, job pozostaje `QUEUED/attempts=0`, gate `False`, marker nie istnieje, a flagi są fail-closed. P2-2 pozostaje jawną, nieblokującą obserwacją false STOP. Etap 1 ma status `OPEN / READY FOR NEW OWNER AUTHORIZATION AFTER STANDALONE QUIESCENCE CHECK`; druga próba nie jest autoryzowana.
...
python -m pytest                # 1174 testy, bez sieci
...
- Kontrolowany live acceptance przechodzi wyłącznie przez `python -m app.main controlled-live-once`. Jedyna autoryzowana próba z 2026-07-17 została zatrzymana przez wewnętrzny preflight przed providerem i nie może być ponawiana. Przed jakąkolwiek nową autoryzacją trzeba uruchomić `controlled-live-quiescence-check` z dokładnie tego samego launchera, zamknąć inne terminale/edytory/shelle zawierające pełny tekst komendy i potraktować każde `PROCESSES_PRESENT` jako `STOP`. Realny wrapper wymaga wcześniej utrwalonego, dokładnie jednego claimable joba z zatwierdzonym frozen pricing contract; jedynym operatorskim kodem otwierającym flagi paid/worker pozostaje ten wrapper. Bieżący gate jest `False`, a flags fail-closed.
