...
- Zbudowane i przetestowane offline: konfiguracja, SQLite z **14 migracjami** (ostatnia `0014_provider_attempt_reconciliation`), Policy Engine, kolejka/worker, ledger provider attempt oraz durable single-research `durable_provider_v2`. `model_usage` pozostaje jedynym ledgerem kosztu; operatorski resolver L1 rozdziela fakt finansowy od wyniku wykonawczego, z append-only historią `reconciliation_events`, pełną weryfikacją lineage i bez retry/attemptu #2. Recovery eskaluje crash-window z martwym lease, a workerowe failure boundaries używają centralnej operacji StoragePort: bez active attemptu zwykłe `FAILED`, przy `RESERVED`/`REQUEST_STARTED` widoczne reconciliation z zachowaną rezerwacją. SQLite blokuje terminalne job/run/research_run przy active attempt. **1036 testów, wszystkie offline.**
- WAVE 0A, WAVE 0B i WAVE 1A są formalnie **`CLOSED — APPROVED WITH P2`**. WAVE 1A zamknął właściciel 2026-07-16 po niezależnym werdykcie `APPROVE WITH MINOR/P2`; Etap 1 pozostaje `BLOCKED`. `durable_provider_v1` jest historyczny i fail-closed; aktywny jest jeden durable paid-execution flow `durable_provider_v2` z `durable_research_intent_v2`. Live API jest **ZABRONIONE**.
...
python -m pytest                # 1036 testów, bez sieci
