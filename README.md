...
- **ETAP 2 / WAVE E2-A = `CLOSED — APPROVED WITH MINOR/P2`; cały Etap 2 = `IN PROGRESS — E1 CLOSED, E2-A CLOSED, E2-B NOT STARTED` (ADR-103).** Niezależny review `APPROVE WITH MINOR/P2`, merge PR #5 (`404d2d306bbfa24fc08f2f5db68931e7441f040a`, zatwierdzony head `61a509bd9c0a457ac78bb8893438664017a14063`) i post-merge checkpoint **1474/1474** (exact-once `357+361+369+387`) potwierdziły offline evidence integration spine: CLI→enqueue→Worker→Dispatcher→STAGED A1/A2/B→`FakeFetch`→evidence→lokalny verifier→Research Card→atomowa terminalizacja→reopen. Trwały wynik: job `DONE`, run `DRY_RUN` (koszt 0), research_run `COMPLETE`, topic `USED`, 1 Research Card, 3/3 candidates `VERIFIED`, 3 retrievals, 3 excerpts, drugi Worker `IDLE`; provider_attempts/model_usage/reservations/settlements `0`, koszt `0.000000 USD`. Runtime wymaga dokładnie `0017`; produkcyjna DB pozostaje na `0014`, a `0015`–`0017` nie zostały zastosowane. Przyjęto trzy P2 (`E2-A-P2-01` OPEN/BACKLOG, `E2-A-P2-02` ACCEPTED, `E2-A-P2-03` OPEN — FUTURE PAID/LIVE GATE) bez naprawy. E2-B nie rozpoczęte; live, realny Fetch, realny staged provider, controlled-live, browser i publikacja pozostają nieautoryzowane.
