...
- **PR #1 został zmergowany do `main`** jako merge commit `548cc65cad70eaef631fafff7c350845984d18e6`. Historyczny branch `dev/first-successful-research-card` jest technicznie zakończony; dalsza praca powstaje na nowych branchach z aktualnego `main`. Mały checkpoint `fix/post-merge-branch-sensitive-test` usuwa wyłącznie zaszytą w teście starą nazwę brancha, nie zmieniając produkcyjnego branch gate'u. Produkcja nadal ma schema `0014`; migracje `0015`/`0016` wymagają osobnej autoryzacji; Etap 2 ma pierwszą falę E1 w statusie `CANDIDATE COMPLETE — AWAITING INDEPENDENT REVIEW` (ADR-099), a live `NOT AUTHORIZED`.
- Zbudowane i przetestowane offline: konfiguracja, kod SQLite z **16 migracjami** (produkcja nadal zweryfikowana na `0014`/14), Policy Engine, kolejka/worker, ledger provider attempt, durable single-research v3, output-size contract, kanoniczny wrapper `controlled-live-once`, fencing i raporty/recovery bez retry. `0015` domyka execution-only crash po `SETTLED`; `0016` (fala E1 Etapu 2, ADR-099) dodaje IZOLOWANY fundament evidence: `evidence_retrievals`/`evidence_excerpts` z podłogami exact-substring/append-only, jedną kanonizację tekstu (offsety wyłącznie do utrwalonego `canonical_text`), deterministyczny weryfikator, `FetchPort` (fake/disabled — bez realnego adaptera sieciowego) i deterministyczną ekstrakcję HTML→tekst; pipeline researchu NIE jest jeszcze zintegrowany z evidence. Runtime `SqliteStorage.open()` wymaga dokładnie `0016`, nie migruje ani nie tworzy DB: immutable preflight → URI `mode=rw` istniejącego pliku → drugi gate na tym handle → writable PRAGMA. Inicjalizacja oraz jawne kroki `0014→0015` i `0015→0016` są osobnymi operacjami. **1408/1408 testów; partycje exact-once 339+342+356+371; QA schema-gate 21/21, evidence floor 21/21, recovery 4/4, lineage 10/10.**
...
Migracje schema `0014→0015` i `0015→0016` nie są wykonywane podczas startu aplikacji. Jedyne jawne rooty to `python scripts/migrate_schema_0015.py --db-path <PATH> --confirm-0014-to-0015` oraz `python scripts/migrate_schema_0016.py --db-path <PATH> --confirm-0015-to-0016`; każdy wymaga wskazania konkretnego pliku, exact preflightu źródłowej wersji, stosuje dokładnie jeden krok i nie uruchamia runtime. Sam przełącznik CLI nie zastępuje osobnej zgody właściciela, quiescence ani backupu. Produkcyjna baza nie została tymi poleceniami zmigrowana.
...
python -m pytest                # 1408 testów, bez sieci
