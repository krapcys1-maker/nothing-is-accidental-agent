...
> Data weryfikacji: **2026-07-13** (`python -m pytest` → **337 passed** offline; Tasks 1–8 Etapu 0 ukończone; źródłowa baza niezmieniona).
...
- Testy: **337 passed** (offline, deterministyczne, bez sieci; w tym jawny resume audit CAS oraz rzeczywiście równoległe race tests na plikowej SQLite). Gałąź: `dev/first-successful-research-card`.
...
1. **Realny run zadania 9 Etapu 0** (`IMPLEMENTATION_ROADMAP.md`) — Task 8 przeszedł końcowe review bez P0/P1; przed zadaniem 9 nadal wymagana jest osobna, jawna zgoda właściciela na realny run.
...
- **Ostatni ukończony krok:** Etap 0 / zadanie 8 — zwykły terminalny FAILED jest niezmienny, jawny research resume ma osobny helper z run–research–topic–account, flow/status i CAS, a candidate claim ma prawdziwy test równoległy z `Barrier`; końcowe review: **APPROVE**, bez P0/P1; **337 testów**, zero API i koszt 0 USD.
- **Następne trzy zadania:**
  1. Realny run ADR-022 wyłącznie za osobną zgodą właściciela.
  2. Etap 1 dopiero po spełnieniu kryterium zakończenia Etapu 0.
