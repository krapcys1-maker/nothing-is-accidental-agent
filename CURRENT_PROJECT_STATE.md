...
- Testy: **529 passed** (offline, deterministyczne, bez sieci; F4 plus 0009 jobs/system_flags, 53 testy kolejki i 22 testy runtime: atomowy claim, runtime flags bez cache, LOCAL noop, RESEARCH dry-run, ścisły CAS job→run→research_run, recovery przypiętego runu oraz reaper osieroconych RUNNING z Barrier/reopen i CLI na tymczasowej SQLite). Gałąź: `dev/first-successful-research-card`.
...
| Worker loop / scheduler runtime | VERIFIED | 69 | `run_once`, kontrolowane `run_forever`, jawny dispatcher, `worker --once`, LOCAL noop i RESEARCH dry-run; osobne `reap-runs --once` odzyskuje lease i zatrzymuje osierocone runy | live API NOT VERIFIED; paid worker BLOCKED; browser/public worker BLOCKED; cykliczny scheduler reapera NOT_STARTED; przypięty RESEARCH run → NEEDS_VERIFICATION, bez auto-resume; brak okien redakcyjnych | `test_worker_runtime` (22) + pełny suite | 2026-07-13 | scheduler reapera, potem osobne granice paid/browser |
...
Brak blockerów Etapu 0 — kryterium zakończenia spełnione. Etap 1 ma trwały claim, active-topic lock, rezerwację, runtime PolicyEngine, minimalny worker i jawny reaper `runs.status=RUNNING→STOPPED`, wszystko zweryfikowane offline. Reaper najpierw wymaga recovery joba; powiązany RESEARCH pozostaje `NEEDS_VERIFICATION`, a run nigdy nie jest automatycznie wznawiany. Przed każdym płatnym albo browser/public workerem pozostają osobne granice bezpieczeństwa i zgoda właściciela; oba typy są teraz BLOCKED. Cykliczny scheduler reapera pozostaje NOT_STARTED. Karta #2 ma `publication_recommendation=REJECT`, więc nie może zasilić przyszłego content pipeline bez poprawy dowodów. Historyczny run `9bbeb020` i P2-20 pozostają długiem technicznym.
