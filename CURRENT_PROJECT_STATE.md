...
> Data weryfikacji: **2026-07-12** (pełny odczyt kodu + `python -m pytest` → **164 passed** + migracje 0006/0007 sprawdzone na pamięciowej kopii `data/agent.db`; źródłowa baza niezmieniona).
...
- Testy: **164 passed** (offline, deterministyczne, bez sieci; w tym regresje dla conservative backfillu attempts, atomowego claimu, crash-window, wyższego capu, ledgeru migracji i izolacji kont). Gałąź: `dev/first-successful-research-card`.
...
| `scripts/run_capped_research.py` | VERIFIED | 94 | pre-flight deleguje do `PolicyEngine.check_run_budget`; estymata obejmuje `1+max_retries`; estimate-only bez klienta; resume uwzględnia zapisany usage | `--max-sources 0` znaczy „wszyscy" (P2-15) | black-box CLI + budget delegation | 2026-07-12 | Etap 0 zad. 6 |
...
1. **Ukończenie zadań 6–8 Etapu 0** (`IMPLEMENTATION_ROADMAP.md`) — Tasks 1–5 są ukończone; pozostałe naprawy wykonawcze muszą zostać wdrożone, przetestowane i udokumentowane przed kolejnym płatnym uruchomieniem.
...
- **Ostatni ukończony krok:** Etap 0 / zadanie 5 — po self-review realny pipeline bez capu jest fail-closed, resume używa stałego absolutnego capu i waliduje konto przed usage, PolicyEngine odrzuca uszkodzone limity/sumy, a A1/A2/B mają regresje odmowy przed attempt 2; **257 testów**, zero API i koszt 0 USD.
- **Następne trzy zadania:**
  1. Wyrównanie klienta tematów dla parse-error i code fence.
  2. Higiena rejestru decyzji (ADR-001/002/003/005/006).
  3. Dalsze naprawy Etapu 0 zgodnie z roadmapą.
