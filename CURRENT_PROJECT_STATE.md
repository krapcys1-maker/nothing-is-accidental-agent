...
> Data weryfikacji: **2026-07-13** (`python -m pytest` → **512 passed**; Etap 0 zamknięty, a foundation Etapu 1 oraz minimalny worker LOCAL/RESEARCH dry-run są zweryfikowane offline. Przypięty run RESEARCH po expiry wymaga reconciliation i przechodzi do NEEDS_VERIFICATION. Live API = NOT VERIFIED; paid i browser/public worker = BLOCKED; reaper runs = NOT_STARTED).
...
- Testy: **512 passed** (offline, deterministyczne, bez sieci; F4 plus 0009 jobs/system_flags, 38 testów kolejki i 20 testów workera: atomowy claim, runtime flags bez cache, LOCAL noop, RESEARCH dry-run, ścisły CAS job→run→research_run, bezpieczne recovery przypiętego runu, heartbeat, utrata lease, pusty queue backoff, Barrier/reopen i CLI na tymczasowej SQLite). Gałąź: `dev/first-successful-research-card`.
...
| Storage SQLite + 9 migracji | VERIFIED | 99 | repozytoria, flow, WAL/busy timeout, atomowy koszt, claim A2/F4 oraz `0009`: jobs, flags, lease, idempotency, active-topic partial UNIQUE, rezerwacje i ścisły CAS `job.run_id`→research run tego samego account/topic/flow | starsze migracje bez transakcji; reaper `runs` nie istnieje | storage + flow + candidate attempts + finalization + jobs queue + worker concurrency | 2026-07-13 | reaper runs osobno |
...
| CLI `app/main.py` + runner | VERIFIED | 90 | run-topics, run-research (dry), blokada `--real` (P0-3), `worker --once` oraz jawny `--poll-seconds` | brak paid/browser CLI i realnego resume przez worker (celowo) | tak + CLI worker na tymczasowej SQLite | 2026-07-13 | reaper runs osobno |
...
| Porty: Storage/Notification | VERIFIED / WORKING | 97/70 | Storage obejmuje typed jobs, lease, flagi, rezerwacje oraz fail-closed CAS relacji job→run→research_run | reaper runs pozostaje poza kontraktem | `test_jobs_queue` + `test_worker_runtime` | 2026-07-13 | reaper runs osobno |
...
| Porty: Browser/Scheduler | SKELETON / VERIFIED | 10/65 | DisabledBrowser blokuje każdą akcję; jeden trwały worker jest w `app/scheduler/` | browser/public worker pozostaje BLOCKED, brak okien redakcyjnych i reapera | `test_worker_runtime` | 2026-07-13 | Etap 5 / reaper runs |
...
| Task queue storage (jobs/flags/lease/rezerwacja) | VERIFIED | 97 | `0009`, atomowy claim, idempotency, recovery, marker skutku zewnętrznego, NEEDS_VERIFICATION, runtime flags, globalny budżet oraz ścisła relacja `job.run_id` | brak reapera `runs`; przypięty run po expiry wymaga reconciliation, nie resume | `test_jobs_queue` + worker Barrier/reopen | 2026-07-13 | reaper runs osobno |
| Worker loop / scheduler runtime | VERIFIED | 67 | `run_once`, kontrolowane `run_forever`, jawny dispatcher, `worker --once`, LOCAL noop i RESEARCH dry-run przez istniejący pipeline | live API NOT VERIFIED; paid worker BLOCKED; browser/public worker BLOCKED; reaper runs NOT_STARTED; przypięty RESEARCH run → NEEDS_VERIFICATION, bez auto-resume; brak okien redakcyjnych | `test_worker_runtime` (20) + pełny suite | 2026-07-13 | reaper runs, potem osobne granice paid/browser |
...
Brak blockerów Etapu 0 — kryterium zakończenia spełnione. Etap 1 ma trwały claim, active-topic lock, rezerwację, runtime PolicyEngine i minimalny worker zweryfikowany wyłącznie offline. Przed każdym płatnym albo browser/public workerem pozostają osobne granice bezpieczeństwa i zgoda właściciela; oba typy są teraz BLOCKED. Reaper `runs.status=RUNNING→STOPPED` jest NOT_STARTED. Karta #2 ma `publication_recommendation=REJECT`, więc nie może zasilić przyszłego content pipeline bez poprawy dowodów. Historyczny run `9bbeb020` i P2-20 pozostają długiem technicznym.
...
- **ADR-038 (ACCEPTED / zweryfikowane offline):** jeden worker odczytuje runtime flags bez cache, wykonuje tylko LOCAL noop i RESEARCH `dry_run=true`, wiąże `job.run_id` przez CAS; paid/browser pozostają BLOCKED, reaper NOT_STARTED.
...
- **Aktywny etap roadmapy:** Etap 1 — storage foundation, runtime PolicyEngine oraz minimalny worker są zweryfikowane offline. Paid/browser są BLOCKED, live API NOT VERIFIED, reaper runs NOT_STARTED.
...
- **Następne trzy zadania:**
  ...
  2. Reaper osieroconych `runs` jako osobne, offline zadanie.
