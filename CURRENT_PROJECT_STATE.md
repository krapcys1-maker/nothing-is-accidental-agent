...
> Data weryfikacji: **2026-07-13** (`python -m pytest` → **566 passed**; Etap 0 zamknięty, a foundation Etapu 1, minimalny worker LOCAL/RESEARCH dry-run, okresowy guard heartbeat podczas dispatchu oraz jednorazowy reaper RUNNING→STOPPED są zweryfikowane offline. Przypięty run RESEARCH po expiry wymaga reconciliation i przechodzi do NEEDS_VERIFICATION. Live API = NOT VERIFIED; paid i browser/public worker = BLOCKED; cykliczny scheduler reapera = NOT_STARTED).
...
- Testy: **566 passed** (offline, deterministyczne, bez sieci; F4 plus 0009 jobs/system_flags, 53 testy kolejki i **59 testów runtime**: atomowy claim, runtime flags bez cache, LOCAL noop, RESEARCH dry-run, ścisły CAS job→run→research_run, osobne połączenie okresowego heartbeat guard podczas dispatchu, recovery przypiętego runu oraz reaper osieroconych RUNNING z Barrier/reopen i CLI na tymczasowej SQLite). Guard heartbeat ma **26 bezpośrednich testów**: **15** pierwotnych periodic heartbeat oraz **11** bounded lifecycle/P1. Gałąź: `dev/first-successful-research-card`.
...
| Storage SQLite + 9 migracji | VERIFIED | 99 | repozytoria, flow, WAL/busy timeout, atomowy koszt, claim A2/F4 oraz `0009`: jobs, flags, lease, idempotency, active-topic partial UNIQUE, rezerwacje, ścisły CAS `job.run_id`→research run i reaper RUNNING→STOPPED | brak cyklicznego schedulera reapera; starsze migracje bez transakcji | storage + flow + candidate attempts + finalization + jobs queue + worker concurrency | 2026-07-13 | scheduler reapera osobno |
...
| CLI `app/main.py` + runner | VERIFIED | 92 | run-topics, run-research (dry), blokada `--real` (P0-3), `worker --once`, jawny `--poll-seconds` oraz `reap-runs --once --stale-after-seconds` | brak paid/browser CLI, realnego resume przez worker i cyklicznego scheduler reapera | tak + CLI worker/reaper na tymczasowej SQLite | 2026-07-13 | scheduler reapera osobno |
...
| Porty: Storage/Notification | VERIFIED / WORKING | 98/70 | Storage obejmuje typed jobs, lease, flagi, rezerwacje, fail-closed CAS relacji job→run→research_run oraz `reap_orphaned_stale_runs` | brak cyklicznego schedulera reapera | `test_jobs_queue` + `test_worker_runtime` | 2026-07-13 | scheduler reapera osobno |
...
| Porty: Browser/Scheduler | SKELETON / VERIFIED | 10/67 | DisabledBrowser blokuje każdą akcję; jeden trwały worker jest w `app/scheduler/`, a reaper jest osobnym CLI | browser/public worker pozostaje BLOCKED, brak okien redakcyjnych i cyklicznego schedulera reapera | `test_worker_runtime` | 2026-07-13 | Etap 5 / scheduler reapera |
...
| Task queue storage (jobs/flags/lease/rezerwacja) | VERIFIED | 98 | `0009`, atomowy claim, idempotency, recovery, marker skutku zewnętrznego, NEEDS_VERIFICATION, runtime flags, globalny budżet, ścisła relacja `job.run_id` oraz reaper runów | reaper wymaga jawnego progu i recovery jobów; przypięty run po expiry wymaga reconciliation, nie resume | `test_jobs_queue` + worker Barrier/reopen | 2026-07-13 | scheduler reapera osobno |
| Worker loop / scheduler runtime | VERIFIED | 72 | `run_once`, kontrolowane `run_forever`, jawny dispatcher, `worker --once`, LOCAL noop i RESEARCH dry-run; podczas dispatchu osobny daemon heartbeat guard odnawia lease co 20 s przy lease 60 s. Worker zawsze wykonuje stop event, `wake`, bounded join i `is_alive()`; zwykle wątek kończy się i jest dołączony, a timeout pozostawia go tylko do odblokowania zależności i blokuje `DONE`. Osobne `reap-runs --once` odzyskuje lease i zatrzymuje osierocone runy | live API NOT VERIFIED; paid worker BLOCKED; browser/public worker BLOCKED; cykliczny scheduler reapera NOT_STARTED; przypięty RESEARCH run → NEEDS_VERIFICATION, bez auto-resume; brak okien redakcyjnych | `test_worker_runtime` (59; heartbeat 15 + 11 = 26) + pełny suite (566) | 2026-07-13 | scheduler reapera, potem osobne granice paid/browser |
...
Brak blockerów Etapu 0 — kryterium zakończenia spełnione. Etap 1 ma trwały claim, active-topic lock, rezerwację, runtime PolicyEngine, minimalny worker, okresowy daemon guard heartbeat na osobnym połączeniu SQLite oraz jawny reaper `runs.status=RUNNING→STOPPED`, wszystko zweryfikowane offline. Daemon jest wyłącznie ostatnią osłoną procesu: worker zawsze ustawia stop event, wywołuje `wake`, wykonuje bounded join i sprawdza `is_alive()`; timeout blokuje `DONE`, a później odblokowany guard widzi stop event przed kolejnym heartbeat. `lost_lease` i `failure` pozostają wyłącznie in-memory, zaś trwałe rozstrzygnięcie joba należy do SQLite oraz recovery/reconciliation. Guard nie robi retry dispatchu; utrata lease albo błąd guarda blokuje DONE, a utrata lease ma pierwszeństwo przed błędem dispatchu. Reaper najpierw wymaga recovery joba; powiązany RESEARCH pozostaje `NEEDS_VERIFICATION`, a run nigdy nie jest automatycznie wznawiany. Przed każdym płatnym albo browser/public workerem pozostają osobne granice bezpieczeństwa i zgoda właściciela; oba typy są teraz BLOCKED. Cykliczny scheduler reapera i okna redakcyjne pozostają NOT_STARTED. Karta #2 ma `publication_recommendation=REJECT`, więc nie może zasilić przyszłego content pipeline bez poprawy dowodów. Historyczny run `9bbeb020` i P2-20 pozostają długiem technicznym.
...
- **Aktywny etap roadmapy:** Etap 1 — storage foundation, runtime PolicyEngine, minimalny worker i jednorazowy reaper są zweryfikowane offline. Paid/browser są BLOCKED, live API NOT VERIFIED, cykliczny scheduler reapera NOT_STARTED.
...
- **Następne trzy zadania:**
  1. Niezależne review typed provider errors/F4 oraz minimalnego runtime Etapu 1; bez API.
  2. Niezależne review reapera osieroconych `runs`; automatyczny scheduler reapera pozostaje osobnym zadaniem.
