...
def build_evidence_research_prompt(
    ...
    return (
        ...
        "working_thesis: string, at most 60 words. "
        "main_mechanism: string or null, at most 100 words. "
        "confirmed_claims: array of 3 to "
        f"{len(contract.documents)} strings, each at most 30 words and each a "
        "load-bearing claim ABOUT THE SUBJECT drawn from one evidence document. "
        "Never state as a confirmed claim anything about a document itself - that "
        "it was unavailable, access-restricted, unreadable, truncated or empty. "
        "That is a fact about our retrieval, not about the subject, and it makes "
        "the card unusable. A document carrying no usable claim is simply left "
        "out; fewer claims from solid documents beat one claim per document. "
        "uncertain_claims: array of at most 3 strings, each at most 30 words. "
        "contradictions: array of at most 3 strings, each at most 35 words. "
        "strongest_counterargument: string or null, at most 60 words. "
        "citable_numbers: array of 3-6 short strings, each at most 15 words; each "
        ...
        "do not create any confirmed claim without source lineage. A claim you "
        "cannot support with a verbatim excerpt copied character for character "
        "from that source must be DROPPED from confirmed_claims entirely - not "
        "kept with a paraphrase, a null excerpt or an approximate quote. A card "
        "whose claim cannot be bound to its excerpt is discarded whole, so three "
        "claims you can quote exactly are worth more than five you cannot. "
        "If the original "
