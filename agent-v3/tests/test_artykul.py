...
wlasne = {p.stem for p in pathlib.Path("agent-v3").glob("*.py")}
moduly = set()
for f in pathlib.Path("agent-v3").glob("*.py"):
...
wymagania = pathlib.Path("agent-v3/requirements.txt").read_text(encoding="utf-8").lower()
