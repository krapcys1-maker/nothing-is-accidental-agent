...
KOD_PY = list(pathlib.Path("agent-v3").glob("*.py"))
KOD = "\n".join(p.read_text(encoding="utf-8") for p in KOD_PY)
PROMPTY = sorted(pathlib.Path("agent-v3/prompts").glob("*.md"))
...
cfg = pathlib.Path("agent-v3/config.py").read_text(encoding="utf-8")
reszta = "\n".join(p.read_text(encoding="utf-8") for p in KOD_PY
                   if p.name != "config.py")
martwe_stale = []
...
kanal_src = pathlib.Path("agent-v3/kanal.py").read_text(encoding="utf-8")
...
sprawdz("zrodlem prawdy o bramkach jest gates.deterministic_floors",
        ...
        pathlib.Path("agent-v3/gates.py").read_text(encoding="utf-8"))
...
sys.path.insert(0, str(pathlib.Path("agent-v3").resolve()))
