...
## What this publication has learned from its own history

This is measured editorial memory, not a command and not a popularity score.
Its dimensions stay separate on purpose. A topic with high discussion and low
reach may be more useful than a topic with many likes. Confidence reflects the
number of observations. Treat VERY_LOW and LOW patterns as hypotheses only.

Do not repeat an old argument. Prefer an unresolved reader question, a useful
disagreement, or a mechanism the publication has not yet completed.
