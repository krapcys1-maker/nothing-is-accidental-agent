...
- **Zależności:** Etap 0.
- **Pliki/moduły:** `app/storage/migrations/0009_jobs_system_flags.sql`, `app/models.py`, `app/ports/storage.py`, `app/storage/repositories.py`, `app/scheduler/`, runtime `PolicyEngine`, `app/main.py` (subkomendy `worker` i `reap-runs`) i bezpieczny punkt wejścia orchestratora — wdrożone i zweryfikowane OFFLINE. Paid/browser, cykliczny scheduler reapera i okna redakcyjne pozostają PLANNED/BLOCKED.
- **Zadania:**
  0. **Blockery przed płatnymi workerami — ograniczone offline (2026-07-13):** (a) typowana taksonomia provider errors/retry; (b) atomowa finalizacja staged B z `0008`; (c) trwała foundation kolejki `0009`; (d) jeden worker/dispatcher dla LOCAL noop i RESEARCH `dry_run=true`; (e) fail-closed relacja job→run→research_run oraz recovery przypiętego runu. **512 testów**, 0 USD, brak API. To nie odblokowuje paid ani browser/public actions.
  1. **WYKONANE OFFLINE, oczekuje na niezależne review:** tabela `jobs` + `system_flags`, modele/port/repozytoria, idempotency key, active research topic constraint, lease/heartbeat/recovery, marker `external_effect_started_at`, ścisły CAS `job.run_id` oraz NEEDS_VERIFICATION dla BROWSER/niepewnego lub już przypiętego research runu; rezerwacja globalnego budżetu.
  2. **WYKONANE OFFLINE:** `run_once()` używa istniejącego atomowego claimu, przejścia LEASED→RUNNING, heartbeatów w punktach kontrolnych oraz CAS terminalizacji. `run_forever()` wymaga jawnego interwału; CLI `worker --once` wykonuje najwyżej jeden job.
  3. **WYKONANE W WARSTWIE STORAGE:** `idempotency_key UNIQUE`; joby BROWSER po wygaśnięciu lease → NEEDS_VERIFICATION, nigdy ponowne wykonanie.
  ...
  5. **WYKONANE OFFLINE:** `PolicyEngine` odczytuje przy każdym jobie pięć runtime flags z SQLite bez cache i fail-closed. `dry_run=false`, paid oraz browser/public kończą się bez wykonania efektem BLOCKED/FAILED.
  6. **WYKONANE OFFLINE:** jawne `reap-runs --once --stale-after-seconds X` najpierw uruchamia recovery jobów, potem w `BEGIN IMMEDIATE` robi CAS `RUNNING→STOPPED` wyłącznie dla stale runu bez joba `QUEUED/LEASED/RUNNING`. Przypięty RESEARCH po expiry zostaje `NEEDS_VERIFICATION`; auto-resume nie istnieje. Cykliczny scheduler reapera pozostaje NOT_STARTED.
...
- **Testy:** dwa workery nie biorą tego samego joba; dwa joby są claime’owane osobno; enqueue/topic/rezerwacja/heartbeat/recovery/complete/cancel mają testy Barrier + osobne połączenia + reopen. Kontrakt `attach_job_run` sprawdza job/run/research_run po account/topic/workflow/flow, owner i lease; recovery różnicuje RESEARCH bez `run_id` (QUEUE) od przypiętego runu (NEEDS_VERIFICATION). Reaper ma stale threshold przekazywany jako argument, blokadę przed recovery wygasłego lease, statusowy CAS, wyścigi z SUCCESS/FAILED oraz dwa osobne procesy SQLite. Worker/runtime ma 22 testy, kolejka 53.
- **Kryteria akceptacji/zakończenia:** **częściowo spełnione offline** — research daje się zakolejkować i wykonać przez workera `dry_run` z trwałym `run_id`; restart bez `run_id` może bezpiecznie wrócić do kolejki, natomiast przypięty run nie jest uruchamiany od początku i wymaga reconciliation w NEEDS_VERIFICATION. Jednorazowy reaper runów jest wdrożony; do zakończenia Etapu 1 pozostają cykliczny scheduler reapera, zasady paid/browser oraz okna redakcyjne.
...
Etap 0 spełnił kryterium zakończenia 2026-07-13. Etap 1 ma zweryfikowany offline foundation `0009`, jeden bezpieczny worker i jednorazowy reaper (ADR-037/038/039/040): atomowy claim, runtime flags, LOCAL noop, RESEARCH `dry_run=true` z trwałym `job.run_id` oraz `RUNNING→STOPPED` po recovery i stale threshold. Przypięty research run po expiry przechodzi do NEEDS_VERIFICATION, bez auto-retry i bez realnego resume. Publiczne finalizery legacy nie mogą finalizować `staged`; robi to wyłącznie atomowy helper z kosztem z `model_usage`. `0008` utrwala force staged runu, a `0009` istnieje dla jobs/system_flags. Cykliczny scheduler reapera, paid worker, browser/public worker i okna redakcyjne pozostają nierozpoczęte albo BLOCKED.
