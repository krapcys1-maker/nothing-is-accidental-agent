...
>
> **ETAP 0 ZAKOŃCZONY. AKTUALNY ETAP: 1 — BLOCKED.** **WAVE 0A = `CLOSED — APPROVED WITH P2`; WAVE 0B = `CLOSED — APPROVED WITH P2`** (po commicie `c25e1254`); **WAVE 1A = `CLOSED — APPROVED WITH P2`** decyzją właściciela z 2026-07-16 po niezależnym finalnym re-review. Formalny baseline to 14 migracji i **1036 testów offline**. Historyczne statusy/wyniki 1007/980/982/894 są wcześniejszymi iteracjami. Kontrolowane logiczne odtworzenie ustanowiło baseline `data/agent.db`; nie odtwarza bitowo pliku sprzed incydentu. Zamknięcie WAVE 1A nie zamyka Etapu 1, nie odblokowuje live API i nie rozpoczyna Etapu 2. Każde realne (płatne lub publikujące) uruchomienie wymaga osobnej, jawnej zgody właściciela — zawsze.
...
Implementer przekazał `CANDIDATE COMPLETE — AWAITING INDEPENDENT REVIEW`. Niezależny reviewer odtworzył 1036/1036, cztery partycje exact-once, `compileall` i `git diff --check`, po czym wykonał 149/149 własnych kontrprób `Worker.run_once`, 36/36 sprawdzeń SQLite floor i 30/30 sprawdzeń recovery/reaper/crash-window. Werdykt `APPROVE WITH MINOR/P2` i rekomendacja `WAVE 1A MAY BE CLOSED WITH MINOR/P2` zostały przyjęte przez właściciela. WAVE 1A jest formalnie `CLOSED — APPROVED WITH P2`; P2-1/P2-2 nie blokują zamknięcia. Etap 1 pozostaje `BLOCKED`, live API `ZABRONIONE`, Etap 2 nie został rozpoczęty.
...
- **Testy — historyczna końcowa iteracja WAVE 0B:** pełny suite **894 passed / collected** offline. Finalna fala WAVE 0B utrwala `durable_research_intent_v2`: kanoniczny snapshot wejść promptu, stage, provider parameters i wersje kontraktu; ponowna asercja sprawdza lifecycle `job→run→research_run→attempt` przed callerem. `durable_provider_v1` i niepełny payload są fail-closed. W0B-REV-06 domyka przepływ `max_tokens`: jedna trwała wartość steruje callerem, estymatą, policy checkiem i rezerwacją; nadwyżka actual cost jest zapisana tylko raz i wymaga reconciliation. W0B-RR-01 domyka jeden kontrakt USD `Decimal(str(value)) → 0.000001/ROUND_HALF_UP` także dla staged aggregation, policy, persisted sum, pipeline i CLI. Baseline `data/agent.db` pozostaje `CAEDDA05B4E9BCA70346031F5812D5EA38C4A7390D1E52E22FDFA12AF4EBFEFB`; nie jest to bitowe odtworzenie wcześniejszego SHA. WAVE 0B miała wtedy status `APPROVED WITH P2 — READY FOR CHECKPOINT` (nie `CLOSED` przed commitem), Etap 1 pozostawał `BLOCKED`, a live API `ZABRONIONE`. **Stan obecny:** 1036 testów offline, 14 migracji, WAVE 0B i WAVE 1A `CLOSED — APPROVED WITH P2`; Etap 1 nadal `BLOCKED`, live API `ZABRONIONE`.
...
**Status historyczny przed checkpointem WAVE 0B: `WAVE 0B APPROVED WITH P2 — READY FOR CHECKPOINT`; Etap 1 = `BLOCKED`; live API = `ZABRONIONE`.** Niezależny końcowy review potwierdził brak findingów MAJOR i CRITICAL; na tym historycznym etapie nie było to formalne `CLOSED` przed commitem checkpointu. `durable_provider_v2` z `durable_research_intent_v2` pozostawał jedynym aktywnym durable paid-execution flow; v1 i stare/niepełne payloady były fail-closed. `0013_provider_attempt_usage_integrity` była wtedy trzynastą i ostatnią migracją. **Stan obecny:** WAVE 0B i WAVE 1A są `CLOSED — APPROVED WITH P2`, `0014_provider_attempt_reconciliation` jest bieżącą czternastą migracją, a baseline wynosi 1036 testów offline.
