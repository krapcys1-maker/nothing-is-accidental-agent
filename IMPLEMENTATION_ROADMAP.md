...
>
> **PR #1 PO NAPRAWIE PR1-MAJ-005 = `CANDIDATE FOR INDEPENDENT RE-REVIEW` (2026-07-18, ADR-097).** Naprawa PR1-MAJ-001 przez `0015_settled_execution_recovery` pozostaje, ale zwykły runtime nie może już tej migracji zastosować: `SqliteStorage.open()` wymaga dokładnie `0015`, nie tworzy bazy i nie uruchamia `apply_migrations()`. Nowa baza i przejście dokładnie `0014→0015` są wyłącznie jawnymi operacjami migracyjnymi; historyczny executor Etapu 1 nadal kończy na `0014`. Dowód: 1328/1328, exact-once `318+322+339+349`, QA schema-gate `8/8`, recovery `4/4`, lineage `10/10`. Produkcja nadal schema `0014`; `0015` nie została zastosowana i wymaga osobnej decyzji. Etap 2 pozostaje `NOT STARTED`, kolejny live `NOT AUTHORIZED`, bez merge.
>
