...
- **Status:** `IN PROGRESS — E1 CLOSED, E2 NOT STARTED` (ADR-101). WAVE E1 = `CLOSED — APPROVED WITH MINOR/P2`: zamknięty zakres obejmuje migrację `0016_evidence_foundation`, tabele `evidence_retrievals`/`evidence_excerpts`, jeden canonical text i offsety wyłącznie względem utrwalonego kanonu, deterministyczny verifier, NUL-free SQLite floor, canonical/claim hash validation, account scope, logiczną idempotencję excerptów, statyczne pomijanie jawnie ukrytych poddrzew HTML, `FetchPort` z `FakeFetch`/`DisabledFetch`, deterministyczną ekstrakcję HTML bez JS, jawny krok `0015→0016` i runtime gate wymagający dokładnie 0016. Podstawa formalnego zamknięcia: pierwszy review `REJECT`, jedna naprawa B01–B04 (ADR-100), niezależny re-review `APPROVE WITH MINOR/P2`, merge PR #3 i post-merge checkpoint 1454/1454. Pipeline researchu NIE jest jeszcze zintegrowany z evidence; E2, realny Fetch, aplikacyjny HTTP, nowy realny request, browser, publikacja i migracje produkcji pozostają nieautoryzowane. Uwaga korygująca do zadań poniżej: numer migracji evidence to `0016` (zapis „migracja 0010" w tej sekcji jest historyczny i nieaktualny), a warunkiem `VERIFIED` będzie lokalny corpus + deterministyczny matcher, nie provider-side web_fetch.
