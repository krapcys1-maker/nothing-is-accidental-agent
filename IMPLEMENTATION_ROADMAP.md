...
- **Zależności:** Etap 0.
- **Pliki/moduły:** `app/storage/migrations/0009_jobs_system_flags.sql`, `app/models.py`, `app/ports/storage.py`, `app/storage/repositories.py`, `app/scheduler/`, runtime `PolicyEngine`, `app/main.py` (subkomendy `worker`, `reap-runs`, `maintain` i dry-run `enqueue-research`) i bezpieczny punkt wejścia orchestratora — wdrożone i zweryfikowane OFFLINE. Paid/browser pozostają BLOCKED; usługa schedulera systemowego i końcowa akceptacja restartu pozostają NOT_STARTED.
- **Zadania:**
  ...
  7. **✅ WYKONANE —** okna redakcyjne przed enqueue: czysty `SchedulingPolicy` czyta wyłącznie jawną konfigurację `growth_policy.editorial_schedule`, waliduje IANA timezone, okna tego samego dnia bez nakładania oraz deterministyczne DST. Zapisuje UTC `earliest_run_at` i zamknięty `schedule_reason`; czas przeszły, brak/niepoprawna konfiguracja i dowolny reason code są odrzucane fail-closed. Atomowy claim wybiera tylko `QUEUED` z `earliest_run_at <= now`, więc job przyszły pozostaje bez lease i attempts. `enqueue-research` tworzy wyłącznie job RESEARCH `dry_run`; nie uruchamia workera, dispatchu, API ani researchu.
     - **⬜ NOT_STARTED —** końcowa akceptacja restartu procesu dla jobów zaplanowanych.
...
- **Testy:** dwa workery nie biorą tego samego joba; dwa joby są claime’owane osobno; enqueue/topic/rezerwacja/heartbeat/recovery/complete/cancel mają testy Barrier + osobne połączenia + reopen. Guard ma **15 pierwotnych** deterministycznych testów periodic heartbeat Event/Barrier bez `sleep` oraz **11** dodatkowych testów bounded lifecycle/P1 (startup/shutdown timeout, factory, heartbeat storage, `close` i `wake` errors); łącznie **26 bezpośrednich testów heartbeat**. Kontrakt `attach_job_run` sprawdza job/run/research_run po account/topic/workflow/flow, owner i lease; recovery różnicuje RESEARCH bez `run_id` (QUEUE) od przypiętego runu (NEEDS_VERIFICATION). Maintenance ma **26** testów literalnych. Scheduling ma **31** testów: wybór/przesunięcie okna, weekend, requested time, IANA/DST, walidacja konfiguracji, UTC/reopen, kontrolowany reason code, eligibility bez lease/attempt, idempotencja i dwa połączenia SQLite z Barrier. Worker/runtime ma **59 passed**, kolejka 53; pełny suite: **623 passed** offline.
- **Kryteria akceptacji/zakończenia:** **częściowo spełnione offline** — research daje się zakolejkować i wykonać przez workera `dry_run` z trwałym `run_id`; restart bez `run_id` może bezpiecznie wrócić do kolejki, natomiast przypięty run nie jest uruchamiany od początku i wymaga reconciliation w NEEDS_VERIFICATION. One-shot i poll maintenance oraz polityka okien redakcyjnych przed enqueue są zweryfikowane offline. Do zakończenia Etapu 1 pozostają końcowa akceptacja restartu zaplanowanych jobów, usługa schedulera systemowego oraz osobne zasady paid/browser.
...
Etap 0 spełnił kryterium zakończenia 2026-07-13. Etap 1 ma zweryfikowany offline foundation `0009`, jeden bezpieczny worker, reaper, osobny `MaintenanceRunner` one-shot/poll oraz politykę harmonogramu redakcyjnego przed enqueue (ADR-037–043): atomowy claim, runtime flags, LOCAL noop, RESEARCH `dry_run=true` z trwałym `job.run_id`, `RUNNING→STOPPED` po recovery/stale threshold oraz `earliest_run_at`/kontrolowany `schedule_reason` w IANA/DST. Job przyszły nie jest claime’owany. Maintenance otwiera osobne SQLite, najpierw odzyskuje lease, następnie reaps stale runy i nie uruchamia workera ani researchu. Przypięty research run po expiry przechodzi do NEEDS_VERIFICATION, bez auto-retry; realne resume pozostaje NOT IMPLEMENTED. Publiczne finalizery legacy nie mogą finalizować `staged`; robi to wyłącznie atomowy helper z kosztem z `model_usage`. `0008` utrwala force staged runu, a `0009` istnieje dla jobs/system_flags; harmonogram używa już istniejących kolumn, bez nowej migracji. Końcowa akceptacja restartu i usługa schedulera systemowego pozostają NOT_STARTED; paid worker i browser/public worker pozostają BLOCKED.
