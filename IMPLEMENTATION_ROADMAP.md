...
- **Zależności:** Etap 0.
- **Pliki/moduły:** `app/storage/migrations/0009_jobs_system_flags.sql`, `app/models.py`, `app/ports/storage.py`, `app/storage/repositories.py`, `app/scheduler/`, runtime `PolicyEngine`, `app/main.py` (subkomenda `worker`) i bezpieczny punkt wejścia orchestratora — wdrożone i zweryfikowane OFFLINE. Paid/browser, reaper runs i okna redakcyjne pozostają PLANNED/BLOCKED.
- **Zadania:**
...
Etap 0 spełnił kryterium zakończenia 2026-07-13. Etap 1 ma zweryfikowany offline foundation `0009` oraz jeden bezpieczny worker (ADR-037/038/039): atomowy claim, runtime flags, LOCAL noop i RESEARCH `dry_run=true` z trwałym, zweryfikowanym `job.run_id`. Przypięty research run po expiry przechodzi do NEEDS_VERIFICATION, bez auto-retry i bez realnego resume. Publiczne finalizery legacy nie mogą finalizować `staged`; robi to wyłącznie atomowy helper z kosztem z `model_usage`. `0008` utrwala force staged runu, a `0009` istnieje dla jobs/system_flags. Reaper `runs`, paid worker, browser/public worker i okna redakcyjne pozostają nierozpoczęte albo BLOCKED.
