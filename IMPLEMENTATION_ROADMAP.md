...
- **Zależności:** Etap 0.
- **Pliki/moduły:** `app/storage/migrations/0009_jobs_system_flags.sql`, `app/models.py`, `app/ports/storage.py`, `app/storage/repositories.py`, `app/scheduler/`, runtime `PolicyEngine`, `app/main.py` (subkomendy `worker`, `reap-runs` i `maintain`) i bezpieczny punkt wejścia orchestratora — wdrożone i zweryfikowane OFFLINE. Paid/browser, usługa schedulera systemowego i okna redakcyjne pozostają PLANNED/BLOCKED.
- **Zadania:**
  ...
  7. **⬜ NOT_STARTED —** okna redakcyjne (godziny działań) z `growth_policy` → filtr `earliest_run_at` oraz runtime `schedule_reason`. Pole `schedule_reason` istnieje w danych joba, ale worker nie wyznacza jeszcze harmonogramu ani nie egzekwuje okien redakcyjnych.
...
Etap 0 spełnił kryterium zakończenia 2026-07-13. Etap 1 ma zweryfikowany offline foundation `0009`, jeden bezpieczny worker, jednorazowy reaper oraz osobny `MaintenanceRunner` one-shot/poll (ADR-037–042): atomowy claim, runtime flags, LOCAL noop, RESEARCH `dry_run=true` z trwałym `job.run_id` oraz `RUNNING→STOPPED` po recovery i stale threshold. Maintenance otwiera osobne SQLite, najpierw odzyskuje lease, następnie reaps stale runy i nie uruchamia workera ani researchu. Przypięty research run po expiry przechodzi do NEEDS_VERIFICATION, bez auto-retry; realne resume pozostaje NOT IMPLEMENTED. Publiczne finalizery legacy nie mogą finalizować `staged`; robi to wyłącznie atomowy helper z kosztem z `model_usage`. `0008` utrwala force staged runu, a `0009` istnieje dla jobs/system_flags. Usługa schedulera systemowego, paid worker, browser/public worker i okna redakcyjne pozostają nierozpoczęte albo BLOCKED.
