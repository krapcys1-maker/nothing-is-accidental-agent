...
- **Zależności:** Etap 0.
- **Pliki/moduły:** `app/storage/migrations/0009_jobs_system_flags.sql`, `app/models.py`, `app/ports/storage.py`, `app/storage/repositories.py` — foundation WDROŻONE OFFLINE; `app/scheduler/` (worker i wybór jobów), runtime `PolicyEngine`, `app/main.py` (subkomenda `worker`) i orchestrator pozostają PLANNED.
- **Zadania:**
  0. **Blockery przed płatnymi workerami — GOTOWE OFFLINE, oczekują na niezależne review (2026-07-13):** (a) typowana taksonomia provider errors/retry; (b) atomowa finalizacja staged B z `0008`; (c) trwała foundation kolejki `0009`: `jobs`, `system_flags`, typed storage, `BEGIN IMMEDIATE` dla enqueue/claim/rezerwacji, lease/recovery, partial UNIQUE active research per topic oraz globalne rezerwacje budżetu. **463 testy**, 0 USD, brak API. Worker loop, scheduler runtime i PolicyEngine runtime flag integration nie powstały.
  ...
  5. **CZĘŚCIOWO WYKONANE:** repozytorium `system_flags` czyta DB runtime i fail-closed; podpięcie PolicyEngine przy KAŻDYM checku (P1-7) pozostaje PLANNED.
...
- **Testy:** dwa workery nie biorą tego samego joba; dwa joby są claime’owane osobno; enqueue/topic/rezerwacja/heartbeat/recovery/complete/cancel mają testy Barrier + osobne połączenia + reopen. Integracja PolicyEngine i reaper `runs` pozostają do wykonania z workerem.
