...
- **Zadania:**
  0. **Blockery przed płatnymi workerami — ograniczone offline (2026-07-13):** (a) typowana taksonomia provider errors/retry; (b) atomowa finalizacja staged B z `0008`; (c) trwała foundation kolejki `0009`; (d) jeden worker/dispatcher dla LOCAL noop i RESEARCH `dry_run=true`. **489 testów**, 0 USD, brak API. To nie odblokowuje paid ani browser/public actions.
  1. **WYKONANE OFFLINE, oczekuje na niezależne review:** tabela `jobs` + `system_flags`, modele/port/repozytoria, idempotency key, active research topic constraint, lease/heartbeat/recovery, marker `external_effect_started_at` i NEEDS_VERIFICATION dla BROWSER/niepewnego skutku oraz rezerwacja globalnego budżetu.
...
- **Testy:** dwa workery nie biorą tego samego joba; dwa joby są claime’owane osobno; enqueue/topic/rezerwacja/heartbeat/recovery/complete/cancel mają testy Barrier + osobne połączenia + reopen. Worker ma 19 testów: lokalny noop, research dry-run, flagi runtime, restart, heartbeat, utrata lease, terminalność, kontrolowane oczekiwanie pustej kolejki i CLI na tymczasowej SQLite.
- **Kryteria akceptacji/zakończenia:** **częściowo spełnione offline** — research daje się zakolejkować i wykonać przez workera `dry_run` z trwałym `run_id`; restart bez skutku zewnętrznego nie gubi joba. Do zakończenia Etapu 1 pozostają reaper `runs`, zasady paid/browser oraz okna redakcyjne.
...
Etap 0 spełnił kryterium zakończenia 2026-07-13. Etap 1 ma zweryfikowany offline foundation `0009` oraz jeden bezpieczny worker (ADR-037/038): atomowy claim, runtime flags, LOCAL noop i RESEARCH `dry_run=true` z trwałym `job.run_id`. Publiczne finalizery legacy nie mogą finalizować `staged`; robi to wyłącznie atomowy helper z kosztem z `model_usage`. `0008` utrwala force staged runu, a `0009` istnieje dla jobs/system_flags. Reaper `runs`, paid worker, browser/public worker i okna redakcyjne pozostają nierozpoczęte albo BLOCKED.
