...
>
> Każde twierdzenie o stanie obecnym w tym dokumencie zostało zweryfikowane w kodzie i testach (**1151 test cases collected and passed offline, 2026-07-17; exact-once 275+282+291+303**) albo przez kontrolowany raport produkcyjnej migracji. Wartości 1127/1079/1052/1036 są historycznymi baseline'ami. Twierdzenia niezweryfikowane oznaczono `NOT VERIFIED`.
>
> **WAVE 0A/0B/1A = `CLOSED — APPROVED WITH P2`; pierwsza WAVE LA-01 = `REJECTED — MAJOR`; LA-01-R1 = `APPROVED WITH MINOR/P2 — CHECKPOINT AUTHORIZED`.** Etap 1 pozostaje **`OPEN / BLOCKED PENDING CONTROLLED LIVE ACCEPTANCE`**. Chroniona produkcyjna baza ma schema `0014`, 14 migracji, kanoniczne flagi fail-closed i baseline SHA `630E3411F2FDFBD232F593DC7E7F3B0DF3EB8125274365815CDBDBC2A3C036A6`. LA-01-R1 wiąże zatwierdzony pricing profile (ID, wersja, model, waluta, jednostki, wszystkie ceny i fingerprint) z durable intentem i dispatcherem; kanoniczny enqueue i jedyny operatorski wrapper współdzielą deterministyczną tożsamość sesji/joba/requestu/attemptu/fence. Wrapper wymaga kompletnego durable evidence, zapisuje sanitizowany raport przed usunięciem markera, wykonuje prawdziwy reopen i recovery bez retry. Otwieranie profilu wymaga atomowego kompletu pięciu flag; pojedynczy setter może wyłącznie zamykać. Jedyny open P2 — rekurencyjna sanitizacja nieosiągalnego fallbacku `str(value)` — jest nieblokującą rekomendacją defense-in-depth i nie jest częścią reviewed diffu. **Realny controlled live acceptance niewykonany i nieautoryzowany** (`REAL_CONTROLLED_LIVE_ENABLED=false`). Live API `ZABRONIONE`; Etap 2 nie został rozpoczęty.
...
| SQLite + 14 migracji + repozytoria | `app/storage/` | `tests/test_storage.py`, `tests/test_research_run_flow.py`, `tests/test_jobs_queue.py`, `tests/test_provider_attempt_reconciliation.py` i in.; `0008` utrwala force staged runu, `0009` dodaje jobs/system_flags, `0010`–`0013` durable provider ledger, a `0014` dodaje audytowalny resolver L1 bez drugiego ledgeru kosztu |
...
| Approved pricing i controlled-live composition root | `app/core/pricing.py`, `app/research/durable_intent.py`, `app/operations/controlled_live.py`, `app/main.py controlled-live-once`, `app/scheduler/dispatcher.py` | Profil approved jest pełnym kontraktem `Decimal`; enqueue/projekcja/intent/wrapper/dispatcher/report używają tej samej ceny. Sesja wiąże expected job/request/attempt/token; sukces wymaga trwałego lifecycle+usage+settlement po prawdziwym reopen. Raport jest trwały przed marker clear, recovery zachowuje `REQUEST_STARTED` bez retry. Real gate wyłączony; testy wyłącznie fake/temp DB |
...
Status: **WAVE 0A/0B/1A formalnie `CLOSED — APPROVED WITH P2`; pierwsza LA-01 = `REJECTED — MAJOR`; LA-01-R1 = `APPROVED WITH MINOR/P2`; Etap 1 = `OPEN / BLOCKED PENDING CONTROLLED LIVE ACCEPTANCE`.** Migracja produkcji `0009→0014`, nowy baseline, inicjalizacja pięciu flag i review LA-01-R1 są wykonane. Pozostają: jeden osobno autoryzowany controlled live durable single flow z twardym capem, `max_retries=0`, jednym jobem i jednym requestem; review trwałego wyniku po live teście; brak MAJOR/CRITICAL; formalna decyzja właściciela. Realny adapter Anthropic może działać wyłącznie po osobnej zgodzie i pełnym pre-flight. Live API pozostaje zabronione.
...
Historyczna weryfikacja WAVE 0A miała 714 testów, WAVE 0B.3 — 770, W0B-RR-01 — 894, zaakceptowany baseline WAVE 1A — 1036, skonsolidowany pakiet — 1052, a QP-01 — 1079. Bieżąca regresja LA-01-R1 to **1151 passed / collected**, bez sieci, API, publikacji ani kosztu. Produkcyjny baseline `data/agent.db` to SHA-256 `630E3411F2FDFBD232F593DC7E7F3B0DF3EB8125274365815CDBDBC2A3C036A6` przy schema 0014; WAVE 0B i WAVE 1A są `CLOSED — APPROVED WITH P2`, pierwsza LA-01 jest `REJECTED — MAJOR`, LA-01-R1 jest `APPROVED WITH MINOR/P2`, Etap 1 pozostaje `BLOCKED`, a live API `ZABRONIONE`.
