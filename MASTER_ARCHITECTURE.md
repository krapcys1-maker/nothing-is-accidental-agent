...
>
> Każde twierdzenie o stanie obecnym w tym dokumencie zostało zweryfikowane w kodzie i testach (**1036 test cases collected and passed offline, 2026-07-16**, po systemowej naprawie `W1A-R4-01` z czwartego niezależnego `REJECTED — MAJOR`; historyczne 1007/980/982 to wcześniejsze iteracje) albo pamięciowej kopii `data/agent.db`. Twierdzenia niezweryfikowane oznaczono `NOT VERIFIED`.
>
> **WAVE 0A = `CLOSED — APPROVED WITH P2`; WAVE 0B = `CLOSED — APPROVED WITH P2`; WAVE 1A = `CLOSED — APPROVED WITH P2`.** WAVE 1A została formalnie zamknięta przez właściciela 2026-07-16 po niezależnym werdykcie `APPROVE WITH MINOR/P2`. Etap 1 pozostaje `BLOCKED`, live API `ZABRONIONE`; Etap 2 nie został rozpoczęty. Migracja 0014 zachowuje append-only reconciliation, jeden kanon kosztu i defense-in-depth lifecycle. 14 migracji, 1036 testów offline. Historyczne **1007/980/982/894** i `READY FOR CHECKPOINT` są zapisami wcześniejszych iteracji.
...
Historyczna weryfikacja WAVE 0A miała 714 testów, a historyczna regresja WAVE 0B.3 — 770. Historyczna regresja po W0B-REV-06 miała 873 testy, po W0B-REV-09/10 — 887, po W0B-RR-01 — **894**, a iteracje WAVE 1A miały 919/948/955 oraz po naprawie `W1A-VERIFY-02` **980 passed / collected** (wszystkie historyczne). Bieżąca regresja to **1036 passed / collected**, bez sieci, API, publikacji ani kosztu. Baseline `data/agent.db` to SHA-256 `CAEDDA05B4E9BCA70346031F5812D5EA38C4A7390D1E52E22FDFA12AF4EBFEFB`; WAVE 0B i WAVE 1A są `CLOSED — APPROVED WITH P2`, Etap 1 pozostaje `BLOCKED`, a live API `ZABRONIONE`.
