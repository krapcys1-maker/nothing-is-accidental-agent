...
> **AKTUALIZACJA ARCHITEKTONICZNA 2026-07-18 (ADR-097, PR1-MAJ-005):** runtime open i migration open są rozdzielone. `SqliteStorage.open()` nigdy nie tworzy bazy ani nie wykonuje migracji; przed writable connect sprawdza istniejący ledger przez `mode=ro&immutable=1`, wymaga dokładnie `0015_settled_execution_recovery`, a po connect ponawia sprawdzenie. Każdy zapisowy composition root dziedziczy ten fail-closed gate. Read-only raporty pozostają dostępne na `0014`. Jawna inicjalizacja nowej bazy i osobny migrator dokładnie `0014→0015` nie uruchamiają runtime; historyczny executor Etapu 1 pozostaje ograniczony do `0014`. Produkcja nadal ma `0014`; `0015` nie została zastosowana.
...
>
> Każde bieżące twierdzenie o kandydacie naprawy zostało zweryfikowane w kodzie i testach (**1328 test cases collected and passed offline; exact-once `318+322+339+349`**) albo przez QA: schema-gate 8/8, settled recovery 4/4 i lineage 10/10. Historyczne 1311/1288/223 nadal dowodzą poprzednich fal i accepted positive-live; nie są bieżącym baseline'em kodu po naprawie. Twierdzenia niezweryfikowane oznaczono `NOT VERIFIED`.
>
...
| SQLite + 15 migracji w kodzie (produkcja: 14) + repozytoria | `app/storage/` | `tests/test_storage.py`, `tests/test_runtime_schema_gate.py`, `tests/test_research_run_flow.py`, `tests/test_jobs_queue.py`, `tests/test_provider_attempt_reconciliation.py`, `tests/test_settled_execution_recovery.py` i in.; `0008` utrwala force staged runu, `0009` dodaje jobs/system_flags, `0010`–`0013` durable provider ledger, `0014` dodaje audytowalny resolver L1 bez drugiego ledgeru kosztu, a kandydacka `0015` — execution-only recovery po `SETTLED`; runtime wymaga dokładnie `0015`, migracja jest wyłącznie jawna, produkcja pozostaje na `0014` |
