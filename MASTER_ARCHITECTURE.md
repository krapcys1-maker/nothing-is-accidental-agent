...
>
> Każde twierdzenie o stanie obecnym w tym dokumencie zostało zweryfikowane w kodzie i testach (**1052 test cases collected and passed offline, 2026-07-16**; 1036 to zaakceptowany historyczny baseline przed skonsolidowanym pakietem) albo na tymczasowej kopii SQLite. Twierdzenia niezweryfikowane oznaczono `NOT VERIFIED`.
>
> **WAVE 0A = `CLOSED — APPROVED WITH P2`; WAVE 0B = `CLOSED — APPROVED WITH P2`; WAVE 1A = `CLOSED — APPROVED WITH P2`.** Skonsolidowany Etap 1 ma status implementera **`CANDIDATE COMPLETE — AWAITING INDEPENDENT REVIEW`**, formalnie nadal `BLOCKED`. Minimalna integracja Windows Task Scheduler i raport operacyjny są zweryfikowane offline, lecz zadania nie są zarejestrowane. Kod zna 14 migracji; chroniona produkcyjna baza nadal ma 9 i nie została zmigrowana. Live API `ZABRONIONE`; Etap 2 nie został rozpoczęty.
...
Status: **WAVE 0A/0B/1A formalnie `CLOSED — APPROVED WITH P2`; Etap 1 = `CANDIDATE COMPLETE — AWAITING INDEPENDENT REVIEW`, nadal formalnie `BLOCKED`.** Zamknięta lista warunków to: review pakietu; osobno zatwierdzona migracja produkcji `0009→0014` i nowy baseline; jawna inicjalizacja flag; jeden kontrolowany live durable single flow z twardym capem, `max_retries=0`, jednym jobem i jednym requestem; review trwałego wyniku; brak MAJOR/CRITICAL; formalna decyzja właściciela. Realny adapter Anthropic może działać wyłącznie po osobnej zgodzie i pełnym pre-flight. Live API pozostaje zabronione w bieżącym stanie.
...
Historyczna weryfikacja WAVE 0A miała 714 testów, WAVE 0B.3 — 770, W0B-RR-01 — 894, a zaakceptowany baseline WAVE 1A — 1036. Bieżąca regresja skonsolidowanego pakietu to **1052 passed / collected**, bez sieci, API, publikacji ani kosztu. Baseline `data/agent.db` to SHA-256 `CAEDDA05B4E9BCA70346031F5812D5EA38C4A7390D1E52E22FDFA12AF4EBFEFB`; WAVE 0B i WAVE 1A są `CLOSED — APPROVED WITH P2`, Etap 1 ma status kandydacki i formalnie pozostaje `BLOCKED`, a live API `ZABRONIONE`.
