...
> **AKTUALIZACJA KANDYDATA 2026-07-22 — F1-BLOCK-01 / `TOPIC_GENERATION` SETTLED execution recovery (ADR-110): `CANDIDATE COMPLETE — READY FOR FINAL RE-REVIEW`; Etap 2 pozostaje `IN PROGRESS`.** `provider_attempt=SETTLED` z dokładnie jednym kanonicznym `model_usage.task='topics'`, poprawnym frozen intentem i zużytą zgodą, lecz bez topiców/generated_topics i bez terminalnego joba/runa, ma teraz tę samą rozdzieloną semantykę finansowo-wykonawczą co RESEARCH: attempt i usage pozostają niemutowalne, a maintenance albo publiczny resolver `CHARGED_KNOWN + EXECUTION_FAILED` dopisuje `EXECUTION_RECOVERY` i atomowo kończy run/job jako `FAILED`, czyści lease, external-effect marker i rezerwację oraz nie otwiera retry. Każda niespójność lineage, tasku, requestu, kosztu, liczby usage, approval, attemptu lub wyniku topiców odpada fail-closed bez mutacji. Polimorficzne podłogi są częścią niezmergowanej migracji `0020`; nie powstała `0021`. Dowód offline: collect/exact-once/full `1821/1821`, 0 skipped, 0 xfail, czyli 23 nowe przypadki ponad baseline `1798`; regresje RESEARCH, durable provider, worker/reaper/maintenance i migracji zielone. Produkcyjna `data/agent.db` pozostała na `0019`, SHA-256 `AE8EFCE9A9036BEDD2DDBE2969552B3A97FA8F9A68588AC4BAC4E0562A716944`, `634880 B`, integrity `ok`, FK `0`, bez WAL/SHM/journal; otwierana wyłącznie `mode=ro&immutable=1`. Zero sieci/API/SDK/browsera/publikacji/kosztu; produkcji nie migrowano. Ten wpis nie jest approvalem, merge'em ani zamknięciem Etapu 2.
