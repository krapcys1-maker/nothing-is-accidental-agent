...
>
> Bieżący baseline całej suity po E2-B (ADR-104, kandydat): **collect/full `1551/1551`, zero skipped, exact-once `1551` unikalnych node ID** (zero duplikatów, zero failures), z czego nowe testy E2-B to `50` (adapter/URL policy/intent/composition gate, `tests/test_controlled_fetch_adapter.py`) + `27` (acceptance/negatywne/lease/fencing/recovery/migracja `0018`, `tests/test_e2b_controlled_fetch_foundation.py`); niezależny harness kontrprób `scripts/harness/e2b_refutation_harness.py` = `13/13` inwariantów obronionych. Poprzedni baseline po WAVE E2-A / merge PR #5 (`1474/1474`, partycje `357+361+369+387`) oraz historyczne `1454`/`1331`/`1288`/`223` dowodzą wcześniejszych fal i nie są bieżącym baseline'em. Twierdzenia niezweryfikowane oznaczono `NOT VERIFIED`.
>
