...
>
> Każde twierdzenie o stanie obecnym w tym dokumencie zostało zweryfikowane w kodzie i testach (**1079 test cases collected and passed offline, 2026-07-16**; 1052 i 1036 to historyczne baseline'y) albo przez kontrolowany raport produkcyjnej migracji. Twierdzenia niezweryfikowane oznaczono `NOT VERIFIED`.
>
> **WAVE 0A = `CLOSED — APPROVED WITH P2`; WAVE 0B = `CLOSED — APPROVED WITH P2`; WAVE 1A = `CLOSED — APPROVED WITH P2`.** Etap 1 pozostaje **`OPEN / BLOCKED PENDING CONTROLLED LIVE ACCEPTANCE`**. Minimalna integracja Windows Task Scheduler i raport operacyjny są zweryfikowane offline, lecz zadania nie są zarejestrowane. Chroniona produkcyjna baza ma 14 migracji po kontrolowanym `0009→0014`, kanoniczne flagi fail-closed i baseline SHA `630E3411F2FDFBD232F593DC7E7F3B0DF3EB8125274365815CDBDBC2A3C036A6`. Live API `ZABRONIONE`; Etap 2 nie został rozpoczęty.
...
Status: **WAVE 0A/0B/1A formalnie `CLOSED — APPROVED WITH P2`; Etap 1 = `OPEN / BLOCKED PENDING CONTROLLED LIVE ACCEPTANCE`.** Migracja produkcji `0009→0014`, nowy baseline i inicjalizacja pięciu flag są wykonane. Niezależny post-migration review zatwierdził QP-01 i zweryfikował schema `0014` oraz baseline wynikiem `APPROVE WITH MINOR/P2`. Pozostają: jeden kontrolowany live durable single flow z twardym capem, `max_retries=0`, jednym jobem i jednym requestem; review trwałego wyniku po live teście; brak MAJOR/CRITICAL; formalna decyzja właściciela. Realny adapter Anthropic może działać wyłącznie po osobnej zgodzie i pełnym pre-flight. Live API pozostaje zabronione.
...
Historyczna weryfikacja WAVE 0A miała 714 testów, WAVE 0B.3 — 770, W0B-RR-01 — 894, zaakceptowany baseline WAVE 1A — 1036, a skonsolidowany pakiet — 1052. Bieżąca regresja QP-01 to **1079 passed / collected**, bez sieci, API, publikacji ani kosztu. Produkcyjny baseline `data/agent.db` to SHA-256 `630E3411F2FDFBD232F593DC7E7F3B0DF3EB8125274365815CDBDBC2A3C036A6` przy schemacie 0014; WAVE 0B i WAVE 1A są `CLOSED — APPROVED WITH P2`, Etap 1 formalnie pozostaje `BLOCKED`, a live API `ZABRONIONE`.
