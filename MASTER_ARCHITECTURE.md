...
>
> Bieżący baseline całej suity po formalnym zamknięciu Etapu 2 / WAVE E1 i merge PR #3 (ADR-101): **collect/full `1454/1454`, zero skipped, exact-once `1454` unikalnych node ID** (rzeczywiste post-merge partycje `352+355+366+381`), oraz QA: evidence foundation `79/79`, evidence migration `44/44`, evidence floor `35/35`, runtime schema gate `21/21`, settled recovery `4/4`, lineage `10/10`. Historyczne `1331`/`1328`/`1311`/`1288`/`223` dowodzą wcześniejszych fal (m.in. RR-01) i nie są bieżącym baseline'em. Twierdzenia niezweryfikowane oznaczono `NOT VERIFIED`.
>
...
| SQLite + 16 migracji w kodzie (`0001`–`0016`, produkcja: 14) + repozytoria | `app/storage/` | `tests/test_storage.py`, `tests/test_runtime_schema_gate.py`, `tests/test_research_run_flow.py`, `tests/test_jobs_queue.py`, `tests/test_provider_attempt_reconciliation.py`, `tests/test_settled_execution_recovery.py`, `tests/test_evidence_foundation.py`, `tests/test_evidence_migration.py` i in.; `0008` utrwala force staged runu, `0009` dodaje jobs/system_flags, `0010`–`0013` durable provider ledger, `0014` dodaje audytowalny resolver L1 bez drugiego ledgeru kosztu, `0015` — execution-only recovery po `SETTLED`, `0016` (WAVE E1, `CLOSED — APPROVED WITH MINOR/P2`, ADR-101) dodaje fundament evidence; runtime `SqliteStorage.open()` wymaga dokładnie `0016` i odrzuca bazę na `0014` albo `0015` jako zbyt starą (`SCHEMA_VERSION_TOO_OLD`) bez automatycznej migracji; `0014→0015` i `0015→0016` pozostają osobnymi, wyłącznie jawnymi operacjami; produkcja pozostaje na `0014`, 14 migracjach — formalne zamknięcie E1 nie autoryzuje migracji produkcyjnej |
...
| **Evidence & citation handling** | każde twierdzenie → źródło + `evidence_excerpt` (cytat z treści źródła po fetch); citable_numbers z kontekstem | wiedza modelu nie zastępuje dowodu (P0-2) | FOUNDATION VERIFIED / PIPELINE INTEGRATION NOT STARTED — `evidence_retrievals`/`evidence_excerpts`, canonical text i jego offsety, deterministyczny verifier, NUL/hash/account/idempotency floor, statyczne pomijanie ukrytego HTML i fake/disabled `FetchPort` są w `main` po WAVE E1 (ADR-101); produkcyjny A2, legacy pipeline'y i durable single NIE zapisują jeszcze evidence excerpts, `verification_status` jest niezmieniony, realny Fetch adapter nie istnieje, a pełny claim/source/run lineage pozostaje do E2 |
