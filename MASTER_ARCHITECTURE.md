...
>
> Każde twierdzenie o stanie obecnym w tym dokumencie zostało zweryfikowane w kodzie i testach (894 test cases collected and passed offline, 2026-07-15) albo pamięciowej kopii `data/agent.db`. Twierdzenia niezweryfikowane oznaczono `NOT VERIFIED`.
...
| SQLite + 13 migracji + repozytoria | `app/storage/` | `tests/test_storage.py`, `tests/test_research_run_flow.py`, `tests/test_jobs_queue.py` i in.; `0008` utrwala force staged runu, `0009` dodaje jobs/system_flags, `0010`–`0013` durable provider ledger; `0013` utrwala fingerprint `execution_intent` dla `durable_provider_v2` |
...
| **Failure recovery** | stany trwałe w SQLite po każdym etapie; wznowienie po restarcie czyta BAZĘ, nie pamięć; jawny reaper osieroconych RUNNING po recovery jobów oraz maintenance one-shot/poll z fail-closed close/factory/recovery/reaper/waiter | brak auto-resume i brak usługi schedulera systemowego | WORKING OFFLINE |
...
Historyczna weryfikacja WAVE 0A miała 714 testów, a historyczna regresja WAVE 0B.3 — 770. Historyczna regresja po W0B-REV-06 miała 873 testy, a po W0B-REV-09/10 — 887. Aktualna regresja po W0B-RR-01 to **894 passed / collected**, bez sieci, API, publikacji ani kosztu. Baseline `data/agent.db` to SHA-256 `CAEDDA05B4E9BCA70346031F5812D5EA38C4A7390D1E52E22FDFA12AF4EBFEFB`; WAVE 0B ma wyłącznie status `CANDIDATE`, a Etap 1 pozostaje `BLOCKED`.
...
**Stan obecny: `WAVE 0B APPROVED WITH P2 — READY FOR CHECKPOINT`; Etap 1 = `BLOCKED`; live API = `ZABRONIONE`.** Niezależny końcowy review potwierdził brak findingów MAJOR i CRITICAL; WAVE nie jest `CLOSED` przed commitem checkpointu. Jedyny wspierany kontrakt to `durable_provider_v2` z `durable_research_intent_v2`; jest to jeden aktywny durable paid-execution flow. v1 oraz każdy niepełny/stary payload są fail-closed. Nie dodano migracji: zmiana jest wersjonowaniem trwałego JSON payloadu, a `0013` pozostaje ostatnią migracją SQLite.
