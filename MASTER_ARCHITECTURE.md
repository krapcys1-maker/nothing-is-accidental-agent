...
| **Evidence & citation handling** | każde twierdzenie → źródło + `evidence_excerpt` (cytat z treści źródła po fetch); citable_numbers z kontekstem | wiedza modelu nie zastępuje dowodu (P0-2) | OFFLINE INTEGRATION CANDIDATE — E2-A łączy prawdziwy CLI/Worker/Dispatcher ze STAGED fake A1/A2/B, `FakeFetch`, E1 verifierem i Research Card; `0017` utrwala pełny minimalny lineage. Legacy/paid staged i realny Fetch nadal nie istnieją. |
