...
>
> Każde twierdzenie o stanie obecnym w tym dokumencie zostało zweryfikowane w kodzie i testach (**1235 test cases collected and passed offline, 2026-07-17; exact-once `294+299+311+331`**) albo przez kontrolowany raport produkcyjnej migracji i controlled-live. Wartości 1200/1181/1174/1151/1127/1079/1052/1036 są historycznymi baseline'ami. Twierdzenia niezweryfikowane oznaczono `NOT VERIFIED`.
>
> **WAVE 0A/0B/1A = `CLOSED — APPROVED WITH P2`; pierwsza WAVE LA-01 = `REJECTED — MAJOR`; LA-01-R1 i LA-02 = `APPROVED WITH MINOR/P2 — CHECKPOINTED`; LA-03 = `APPROVE WITH MINOR/P2`; pierwszy pakiet P2 po LA-03 = `REJECTED — MAJOR`; naprawa NIA-P2-RV-01…05 = `APPROVE WITH MINOR/P2`.** **Etap 1 = `CLOSED`** — formalna decyzja właściciela z 2026-07-17, podstawa: niezależny re-review naprawy NIA-P2-RV-01…05 (zero MAJOR/CRITICAL, pięć findings technicznie zamknięte), zapis w `docs/DECISIONS.md` ADR-088. LA-03 zamknęła drugi false blocker `DB_HANDLES_PRESENT` przez wykonanie canonical handle probe przed otwarciem głównego `SqliteStorage` i trwałą rewalidację po otwarciu. Autoryzowany wrapper wykonał dokładnie jeden provider request: attempt #1 ma `REQUEST_STARTED` i `SETTLED`, usage/koszt `0.053182 USD`, zero retry/attemptu #2. Odpowiedź HTTP 200 zawierała niepoprawny JSON, dlatego job/run/research_run zakończyły się terminalnie `FAILED`, bez Research Card — brak Research Card nie był bramką zamknięcia Etapu 1. Gate `False`, marker nie istnieje, flagi fail-closed. **Zamknięcie Etapu 1 nie autoryzuje kolejnego live: nowy request wymaga oddzielnej, jawnej decyzji właściciela i nowego joba; terminalnego joba nie wolno retry'ować**; Etap 2 nie został rozpoczęty.
