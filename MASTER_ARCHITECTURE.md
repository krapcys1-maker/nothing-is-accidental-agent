...
> **TECHNICZNY SNAPSHOT 2026-07-24 — WAVE C3 = `C3 CODE MERGED — POST-MERGE VERIFIED — AWAITING DOCUMENTATION SYNC MERGE`; ETAP 3 = `IN PROGRESS`.** Provider-ready writer boundary C3 jest zmergowany na `main` i zweryfikowany post-merge; formalne zamknięcie C3 czeka wyłącznie na niezależny review i merge tego dokumentacyjnego PR. Rozdzielony łańcuch dowodu: implementacja → testy implementera → pełny niezależny review `APPROVE WITH MINOR/P2` → poprawka MINOR podniosła C2+C3 z `47/47` na `48/48` → wąski re-review `APPROVE` → commit implementacji `a3096355ab0a648805099f2bfd30ab5d87600fbc` (`feat: add provider-ready content writer boundary`) → PR #24 → merge commit `81936476ad4425e959730f4425c979f1671ef4f1` (rodzice `54c761d0d6c1b7b7402d89e9c24db169b694f00a` i `a3096355ab0a648805099f2bfd30ab5d87600fbc`) → `main` zsynchronizowany wyłącznie ff-only. Architektura pozostaje bez zmian względem kandydata: rozdzielone logical route key/provider/techniczny model ID/pricing/status konfiguracji; ARTICLE=`FABLE_5_ARTICLE`, NOTE=`SONNET_5_NOTE`, fallback=`FORBIDDEN`; leniwe SDK, `max_retries=0`, timeout ≤30 s, brak fallbacku; `0023_provider_ready_writer` zachowuje C2 rows i dodaje append-only typowany wynik providera; wspierany production composition root nadal odmawia, `CONTROLLED_PROVIDER` niedopuszczony przez C3 storage. Post-merge na `main`: C3 `26/26`, C2+C3 `48/48`, celowana regresja `463/463`, pełna suita `1971/1971`, collect/exact case-sensitive unique `1971`, duplikaty/skipped/xfail/errors `0`, `compileall` i `git diff --check` PASS; koszt `0.000000 USD`. Kod na `main` wymaga schema `0023_provider_ready_writer`; produkcja pozostaje byte-identical na `0020_topic_generation_lifecycle` i nie została zmigrowana, runtime wobec niej fail-closed. Nie wykonano realnego API/SDK, prawdziwego artykułu, controlled-live, browsera, publikacji ani migracji produkcji. Osiem P2 pozostaje otwartych, a P2-1/P2-2/P2-3 są wymaganymi gate'ami przed C5. C4 i C5 pozostają `NOT STARTED`; `C3 FULLY CLOSED` nie jest jeszcze ogłoszony.
...
| SQLite + 18 migracji w kodzie (`0001`–`0018`, produkcja: 14) + repozytoria | `app/storage/` | `0016` (E1) utrwala retrieval/excerpt; addytywne `0017` (E2-A) dodaje append-only candidate/retrieval/excerpt/card-source lineage; addytywne `0018` (E2-B, ADR-104) dodaje `controlled_fetch_approvals`/`controlled_fetch_attempts` oraz trigger zamrażający payload `controlled_fetch_v1`. Runtime wymaga dokładnie `0018`; kroki `0014→0015`…`0017→0018` są jawne i osobne. Produkcja pozostaje na `0014`, 14 migracjach. |
...
- Tabele bez żadnego kodu, który je czyta/pisze (schemat od migracji 0001): `content_items`, `interactions`, `target_items`, `approvals`, `metrics_daily`, `screenshots`. Uwaga: E2-B (ADR-104) NIE używa historycznej tabeli `approvals` — zgoda L1 kontrolowanego pobrania żyje w osobnej, aktywnej tabeli `controlled_fetch_approvals` (migracja `0018`), bo tamten schemat nie potrafił wyrazić jednorazowej, wygasającej, nieprzenaszalnej zgody wiązanej fingerprintem intentu.
